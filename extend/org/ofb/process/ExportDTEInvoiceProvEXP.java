 ...
 */
public class ExportDTEInvoiceProvEXP extends SvrProcess
{
	/** Properties						*/
	...
	public String CreateXML(MInvoice inv)
    {
        ...
        try
        {
            ...
            MPaymentTerm pterm = new MPaymentTerm(getCtx(), inv.getC_PaymentTerm_ID(), get_TrxName());
            ...
            MUser salesUser = new MUser(getCtx(), inv.getSalesRep_ID(),get_TrxName());
            ...
            mylog = "Emisor";
            MOrg company = MOrg.get(getCtx(), inv.getAD_Org_ID());
            ...
            MBPartnerLocation bloc = new MBPartnerLocation(getCtx(), inv.getC_BPartner_Location_ID(), get_TrxName());
            ...
            org.w3c.dom.Text contacto = document.createTextNode(getAD_User_ID()>0?inv.getAD_User().getName():" "); //nombre completo contacto
            ...
            BigDecimal amountex = DB.getSQLValueBD(get_TrxName(), (new StringBuilder()).append("select Round(COALESCE(SUM(il.LineNetAmt),0),0) from C_InvoiceLine il  inner join C_Tax t on (il.C_Tax_ID=t.C_Tax_ID) and t.istaxexempt='Y' and il.C_Invoice_ID=").append(inv.getC_Invoice_ID()).toString());
            BigDecimal amountNeto = DB.getSQLValueBD(get_TrxName(), (new StringBuilder()).append("select Round(COALESCE(SUM(il.LineNetAmt),0),0) from C_InvoiceLine il  inner join C_Tax t on (il.C_Tax_ID=t.C_Tax_ID) and t.istaxexempt='N' and il.C_Invoice_ID=").append(inv.getC_Invoice_ID()).toString());
	        ...
	        org.w3c.dom.Text tpoMonedaTxt = document.createTextNode(moneda);
            ...
            //nuevos campos para exportacion
	        ...
	        org.w3c.dom.Text tpoMonedaTxtOM = document.createTextNode("PESO CL");
	        ...
	        org.w3c.dom.Text tpoCambioTxt = document.createTextNode(amtTC.setScale(0, 4).toString());
	        ...
	        org.w3c.dom.Text mntExeOtrMndaTxt = document.createTextNode(amountex != null ? amountex.multiply(amtTC).setScale(0,RoundingMode.HALF_EVEN).toString() : "0");
	        ...
	        org.w3c.dom.Text mntTotOtrMndaTxt = document.createTextNode(inv.getGrandTotal() != null ? inv.getGrandTotal().multiply(amtTC).setScale(0,RoundingMode.HALF_EVEN).toString() : "0");
            ...
            for(int i = 0; i < iLines.length; i++)
            {
                ...
                org.w3c.dom.Text qt = document.createTextNode(iLine.getQtyInvoiced().setScale(4).toString());
                QtyItem.appendChild(qt);
                Detalle.appendChild(QtyItem);
                Element PrcItem = document.createElement("PrcItem");
                org.w3c.dom.Text pa = document.createTextNode(iLine.getPriceActual().multiply(amtTC).setScale(0,RoundingMode.HALF_EVEN).toString());
                PrcItem.appendChild(pa);
                Detalle.appendChild(PrcItem);
                Element MontoItem = document.createElement("MontoItem");
                org.w3c.dom.Text tl = document.createTextNode(iLine.getLineNetAmt().multiply(amtTC).setScale(0,RoundingMode.HALF_EVEN).toString());
            ...
            if(inv.get_Value("C_RefDoc_ID") != null && ((Integer)inv.get_Value("C_RefDoc_ID")).intValue() > 0)//referencia factura
            {
                ...
                MInvoice refdoc = new MInvoice(getCtx(), ((Integer)inv.get_Value("C_RefDoc_ID")).intValue(), get_TrxName());
            ...
            if(inv.get_Value("C_RefInOut_ID") != null && ((Integer)inv.get_Value("C_RefInOut_ID")).intValue() > 0)//referencia despacho
            {
            	 ...
            	 MInOut refdoc = new MInOut(getCtx(), ((Integer)inv.get_Value("C_RefInOut_ID")).intValue(), get_TrxName()); 
            	 tiporeferencia = "52";
                 folioreferencia = (String) refdoc.getDocumentNo();
                 fechareferencia = refdoc.getMovementDate().toString().substring(0, 10);
            	 tipo_Ref = 3; //despacho
            }
            /*if(inv.get_ValueAsString("POReference2") != null && inv.get_ValueAsString("POReference2").length() > 0)//referencia 802
            {
            	 mylog = "referencia:802";
            	 //MOrder refdoc = new MOrder(invoice.getCtx(), ((Integer)get_Value("C_RefOrder_ID")).intValue(), invoice.get_TrxName()); 
            	 tiporeferencia = "802";
                 folioreferencia = inv.get_ValueAsString("POReference2");
                 fechareferencia = inv.getDateOrdered().toString().substring(0, 10);
            	 tipo_Ref = 4; //802
            }*/
          ...
          //se crea segunda referencia para que aparescan 
            if(inv.get_ValueAsString("POReference2") != null && inv.get_ValueAsString("POReference2").length() > 0)//referencia 802
            {
                ...
                org.w3c.dom.Text ref2 = document.createTextNode(inv.get_ValueAsString("POReference2"));
                ...
                if ( CodRefTxt != null && CodRefTxt.length()>0)
                {
                    ...
                    org.w3c.dom.Text codref2 = document.createTextNode(inv.get_ValueAsString("CodRef")==null?"0":inv.get_ValueAsString("CodRef"));                
