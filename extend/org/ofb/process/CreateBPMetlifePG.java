 ...
 */
public class CreateBPMetlifePG extends SvrProcess
implements ImportProcess
{
	...
	public Boolean asignBPartner() throws SQLException
	{
		...
		sql = new StringBuffer ("SELECT C_Bpartner_ID, I_BPartnerXML FROM I_BPartnerXML " +
		...
		try
		{
			...
			{	
				...
				X_I_BPartnerXML ixml = new X_I_BPartnerXML(getCtx(), rs.getInt("I_BPartnerXML"), get_TrxName()); 
	...
	{	
		...
		String sqlCLines = "SELECT C_CampaignFollow_ID FROM C_CampaignFollow WHERE C_Campaign_ID = ? " +
				" AND FinalStatus IN ('AS','CO','IN') Order By C_CampaignFollow_ID";
		...
		try
		{
			...
			while (rsCLines.next())
			{
				...
				String sqlEtBP = "SELECT R_InterestArea_ID, R_InterestAreaValues_ID FROM R_ContactInterest "+
				...
				while (rsEtBP.next())
				{
					...
					if (contadorBP == 1)
					{
						InterestAreaBP1_ID = rsEt.getInt("R_InterestArea_ID");
						InterestAreaValueBP1_ID = rsEt.getInt("R_InterestAreaValues_ID");
					}						
					else if (contadorBP == 2)
					{
						InterestAreaBP2_ID = rsEt.getInt("R_InterestArea_ID");
						InterestAreaValueBP2_ID = rsEt.getInt("R_InterestAreaValues_ID");
					}
					else if (contadorBP == 3)
					{
						InterestAreaBP3_ID = rsEt.getInt("R_InterestArea_ID");
						InterestAreaValueBP3_ID = rsEt.getInt("R_InterestAreaValues_ID");
					}
				}
				...
				if (InterestAreaBP1_ID > 0 )
					id_IaBPsql = Integer.toString(InterestAreaBP1_ID);
				...
				String sqlEtUser = "SELECT DISTINCT(AD_User_ID) FROM R_UserInterest "+
						"WHERE R_InterestArea_ID IN ("+id_IaBPsql+") AND (R_InterestAreaValues_ID IN ("+id_IaVBPsql+") OR R_InterestAreaValues_ID IS NULL) ";
				if (InterestAreaBP2_ID > 0)
				{
				...
				pstmtEtUser = DB.prepareStatement(sqlEtBP, get_TrxName());
