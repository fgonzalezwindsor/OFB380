...
//			Stored Product
		...
		MStorage[] storages, boolean force)
	{
		...
		for (int i = 0; i < storages.length; i++)
		{
			...
			log.config("storage "+i+",id:"+ storage.getM_Locator_ID() +" QtyOnHand:"+storage.getQtyOnHand());
			...
			BigDecimal total = getTotalLocator(storage.getM_Locator_ID(),orderLine.getM_Product_ID(),orderLine.getM_AttributeSetInstance_ID() );
			...
			if (line == null)	//	new line
			{
				...
				if(i+1<storages.length){
					MLocator locTemp = MLocator.get(getCtx(), storages[i+1].getM_Locator_ID());
				...
				if(i+2<storages.length){
					MLocator locTemp = MLocator.get(getCtx(), storages[i+2].getM_Locator_ID());
					pickupDesc+= "/ " + locTemp.getValue() + " C:"+ getTotalLocator(locTemp.getM_Locator_ID(),orderLine.getM_Product_ID(),orderLine.getM_AttributeSetInstance_ID() );
				}
