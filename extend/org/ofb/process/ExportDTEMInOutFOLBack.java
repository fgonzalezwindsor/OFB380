 ...
 */
public class ExportDTEMInOutFOLBack extends SvrProcess
{
	/** Properties						*/
        ...
        try
        {
          ...
          //ininoles indicaciones traslado
            ...
            */
            ...
            BigDecimal taxVariable = Env.ZERO;
            BigDecimal taxFixed = Env.ZERO;
            BigDecimal taxVariableAmt = Env.ZERO;
            BigDecimal taxFixedAmt = Env.ZERO;
            ...
            if (inOut.getC_Order_ID() > 0)
            {
                ...
                {	
            		...
            		MInOutLine iLine = iLines2[a];            		
           			priceT = iLine.getC_OrderLine().getPriceEntered().multiply(iLine.getQtyEntered());
           			if(iLine.getC_OrderLine_ID() > 0)
           			{
               			MOrderLine oLine = new MOrderLine(getCtx(), iLine.getC_OrderLine_ID(), get_TrxName());
               			taxVariable = (BigDecimal)oLine.get_Value("FixedTax");
               			taxFixed = (BigDecimal)oLine.get_Value("VariableTax");
               			if(taxVariable != null)
               				taxVariableAmt = taxVariableAmt.add(taxVariable.multiply(iLine.getQtyEntered()));
               			if(taxFixed != null)
               				taxFixedAmt = taxFixedAmt.add(taxFixed.multiply(iLine.getQtyEntered()));
           			}
            		if(iLine.getC_OrderLine().getC_Tax_ID() > 0)
            		{
            			if (iLine.getC_OrderLine().getC_Tax().getRate().compareTo(Env.ZERO) > 0)
            			{
            				priceT = priceT.setScale(0, RoundingMode.HALF_EVEN);            				
            				taxIVAAmtAcu = priceT.multiply(iLine.getC_OrderLine().getC_Tax().getRate());
            				taxIVAAmtAcu = taxIVAAmtAcu.divide(Env.ONEHUNDRED);
            			}
            	...
            	taxNetoAmtAcu = taxNetoAmtAcu.setScale(0, RoundingMode.HALF_EVEN);
            	//taxIVAAmtAcu = taxIVAAmtAcu.setScale(0, RoundingMode.HALF_EVEN);
            	//amountGrandT = amountGrandT.setScale(0, RoundingMode.HALF_EVEN);
            	amountGrandT = taxNetoAmtAcu.add(taxIVAAmtAcu);
            	if(taxVariableAmt != null)
            		//taxVariableAmt = taxVariableAmt.setScale(0, RoundingMode.HALF_EVEN);
            		amountGrandT = amountGrandT.add(taxVariableAmt);
            	if(taxFixedAmt != null)
            	{
            		//taxFixedAmt = taxFixedAmt.setScale(0, RoundingMode.HALF_EVEN);
            		amountGrandT = amountGrandT.add(taxFixedAmt);
            	}
            	amountGrandT = amountGrandT.setScale(0, RoundingMode.HALF_EVEN);
            }
            ...
            //fin referencia            
            String fullDesc = DB.getSQLValueString(inOut.get_TrxName(), "SELECT MAX(mp.Description) " +
            		" FROM m_inoutline miol " +
            		" INNER JOIN m_product mp ON (miol.m_product_id = mp.m_product_id) " +
            		" WHERE miol.m_inout_id = ? ",inOut.get_ID());
            if(fullDesc != null && fullDesc.trim() != "" && fullDesc.trim() != " ")
            	fullDesc = fullDesc+" "+inOut.getDescription();  	
            else
            ...
            {            
	            ...
	            org.w3c.dom.Text a6Text = document.createTextNode(fullDesc);
