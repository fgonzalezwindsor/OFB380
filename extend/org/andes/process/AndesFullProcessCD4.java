 ...
 */
public class AndesFullProcessCD4 extends SvrProcess
{
	/** Properties						*/
	...
	{	
		...
		//se ejecuta solo si tiene imacro 
		if(aCr.get_ValueAsString("FileNameCD4") != null && aCr.get_ValueAsString("FileNameCD4").trim().length() > 2)
		{
			...
			ejecucion(aCr.get_ValueAsString("FileNameCD4"),rutaBat);
		...
		String[ ] cuentasCorp = new String[2];
		cuentasCorp[0] = "1094016";
		cuentasCorp[1] = "27477217";
		...
		{
			String Val_FileName = DB.getSQLValueString(get_TrxName(),  "SELECT MAX(FileName) FROM C_BankAccount WHERE IsActive = 'Y' " +
					" AND FileName IS NOT NULL AND C_Bank_ID = ?",bank.get_ID());
			String Val_FileNameNew = DB.getSQLValueString(get_TrxName(),  "SELECT MAX(FileNameNew) FROM C_BankAccount WHERE IsActive = 'Y' " +
					" AND FileName IS NOT NULL AND C_Bank_ID = ?",bank.get_ID());
			...
			if(Val_FileName != null && Val_FileName.trim().length() > 3)
			{
		        ...
		        //seteamos nueva ruta
		        String pFile = ruta+comodin+Val_FileName.replace("--",Integer.toString(ia));
		        if(bank.getDescription().compareToIgnoreCase("BCI") == 0)
		        {
		        	...
		        	pFile = ruta+comodin+Val_FileName.replace("-MESDIA-",fechaStrBCI);
		        }
		        if(bank.getDescription().compareToIgnoreCase("security") == 0)
			    ...
			    try 
			    {
			        ...
			        else if(bank.getDescription().compareToIgnoreCase("BCI") == 0)
				    {
					    ...
					    {	
					    	...
					    	if(numAccount.compareToIgnoreCase(datos.get(x)[0].substring(5, 13)) != 0)
					    	{
							    ...
							    String rutaArchivo = OFBForward.PathDataOutIMacroCD4()+comodin+Val_FileNameNew.replace("FECHA", fechaStr).replace("--",rellenarCadena(Integer.toString(indice), 2, "0")).replace("-CCTA-", datos.get(x)[0].substring(5, 13));
				    ...
				    {
					    //se genera nuevo archivo
					    String rutaArchivo = OFBForward.PathDataOutIMacroCD4()+comodin+Val_FileNameNew.replace("FECHA", fechaStr);
				    	File archivo = new File(rutaArchivo);
				    	BufferedWriter bw;
				    ...
				    {   
					    ...
					    {	
					    	...
					    	if(numAccount.compareToIgnoreCase(datos.get(x)[0].substring(0, 11)) != 0)
					    	{
					    		...
					    		indice++;
					    		//se genera nuevo archivo					        
							    String rutaArchivo = OFBForward.PathDataOutIMacroCD4()+comodin+Val_FileNameNew.replace("FECHA", fechaStr).replace("--",rellenarCadena(Integer.toString(indice), 2, "0")).replace("-CCTA-",datos.get(x)[0].substring(0,11));
						    	File archivo = new File(rutaArchivo);					    	
						    	//borrado de archivo si ya existe
						    	if(archivo.exists()) 
						    	      archivo.delete();
				    ...
				    {
					    //se genera nuevo archivo
					    String rutaArchivo = OFBForward.PathDataOutIMacroCD4()+comodin+Val_FileNameNew.replace("FECHA", fechaStr).replace("_", Integer.toString(ia));
				    	File archivo = new File(rutaArchivo);
				    	BufferedWriter bw;
				    	//fecha de hoy para comparar despues
				    	...
				    	for (int x = 4; x < cantLines; x++)
					    {
					    	...
					    	if(x >= 10)
					    	{
					    		...
					    		if(datos.get(x)[0] != null && !datos.get(x)[0].trim().toUpperCase().contains("SIN MOV"))
						    	{
						    		...
						    		//newLine.append(numCartola);// ininoles nuevo numero de cartola es en base a parametro
				    ...
				    {
					    //se genera nuevo archivo
					    String rutaArchivo = OFBForward.PathDataOutIMacroCD4()+comodin+Val_FileNameNew.replace("FECHA", fechaStr).replace("--",Integer.toString(ia)).replace("-CCTA-", cuentasSec[ia-1]);
				    	File archivo = new File(rutaArchivo);
				    	BufferedWriter bw;
				    	//borrado de archivo si ya existe
				    	if(archivo.exists()) 
