 ...
 */
public class VInOutGenOFBTotal extends CPanel
		...
		ChangeListener, TableModelListener, ASyncProcess
{
	...
	/**	Logger			*/
	private static CLogger log = CLogger.getCLogger(VInOutGenOFBTotal.class);
	//
	...
	void jbInit() throws Exception
	{
		...
		tabbedPane.add(selPanel, "Seleccion para DesConsolidado");
		...
		//faaguilar OFB begin
		selNorthPanel.add(dateFromLabel, null);
		selNorthPanel.add(dateFromField, null);
		selNorthPanel.add(dateToLabel, null);
		selNorthPanel.add(dateToField, null);
		//faaguilar OFB end
	...
	private void dynInit()
	{
		...
		miniTable.addColumn("Vendedor");//faaguilar OFB
		miniTable.addColumn("Regla Desp");//faaguilar OFB
		...
		if(VLocationDialog.getZones(Env.getCtx()).length>0){//faaguilar OFB
			...
			miniTable.setColumnClass(9, String.class, true, "Vendedor");//faaguilar OFB
			miniTable.setColumnClass(10, String.class, true, "Regla Desp");//faaguilar OFB
			miniTable.setColumnClass(11, BigDecimal.class, true, Msg.translate(Env.getCtx(), "TotalLines"));
		}else{
		...
		miniTable.setColumnClass(8, String.class, true, "Vendedor");//faaguilar OFB
		miniTable.setColumnClass(9, String.class, true, "Regla Desp");//faaguilar OFB
		miniTable.setColumnClass(10, BigDecimal.class, true, Msg.translate(Env.getCtx(), "TotalLines"));
		}
		//
		miniTable.autoSize();
	...
	//  Create SQL
        StringBuffer sql = new StringBuffer(
            "SELECT C_Order_ID, o.Name, dt.Name, ic.DocumentNo, bp.Name, DateOrdered, TotalLines, ic.Description, ic.C_BPartner_ID,ic.salesname,ic.DeliveryRule " //faaguilar OFB add Description , C_bPartner_ID
        ...
        sql.append(" And Upper(ic.documentname) not like '%PRE-VENTA%' ");
        ...
        //
        sql.append(" ORDER BY ic.DocumentNo asc ,DateOrdered asc");
	...
	private void executeQuery()
	{
		...
		try
		{
			...
			while (rs.next())
			{
				...
				miniTable.setValueAt(rs.getString(10), row, ++column); //salesman  faaguilar OFB
				miniTable.setValueAt(rs.getString(11), row, ++column); //deliveryRUle faaguilar OFB
	...
	public void vetoableChange(PropertyChangeEvent e)
	{
		...
		if (e.getPropertyName().equals("DateFrom"))
			from =  (Timestamp) e.getNewValue();
		if (e.getPropertyName().equals("DateTo"))
		    to =  (Timestamp) e.getNewValue();
		executeQuery();
	}	//	vetoableChange
	...
	private void generateShipments ()
	{
        ...
        AD_Process_ID = DB.getSQLValue(trx.getTrxName(), "select AD_Process_ID from AD_Process where Name='InOutGenerateOFBTotal'");
        if(AD_Process_ID<=0){
		...
		//faaguilar OFB add Parameter - ConsolidateDocument ='N'
		ip = new MPInstancePara(instance, 40);
		ip.setParameter("ConsolidateDocument", "Y");
		if (!ip.save())
		{
			String msg = "No Parameter added";  //  not translated
			info.setText(msg);
