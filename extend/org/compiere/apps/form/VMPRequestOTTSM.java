...
import org.compiere.model.X_MP_OT_Request;
 ...
 */
public class VMPRequestOTTSM extends CPanel
	implements FormPanel, ActionListener, TableModelListener, VetoableChangeListener, ASyncProcess
{
	...
	/**	Logger			*/
	private static CLogger log = CLogger.getCLogger(VMPRequestOTTSM.class);
...
//				,GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(5, 0, 5, 5), 0, 0));
	...
	{		
		...
		StringBuffer sql = new StringBuffer("select rt.DATEDOC,rt.DATEREQUIRED,rt.DOCUMENTNO,rt.MP_OT_REQUEST_ID, rt.AD_Org_ID, rt.AD_User_ID, "+
			"rt.A_Asset_ID, rt.MP_JOBSTANDAR_ID, rt.Description, u.name, a.value||'-'||a.name as AssetName, jo.name as jobName,rl.name as PriorityRule, rt.OT_Request_Type "+
	...
	public boolean createOT(int Job_ID, Timestamp Datetrx,String description,int Asset_ID, int Req_ID ){
		...
		X_MP_OT_Request req = new X_MP_OT_Request(Env.getCtx(), Req_ID, null);		
		int mainDetFlag = 0;
		if(req.get_ValueAsInt("MP_MaintainDetail_ID") > 0)
		{
			newOT.set_CustomColumn("MP_MaintainDetail_ID", req.get_ValueAsInt("MP_MaintainDetail_ID"));
			X_MP_MaintainDetail mainDet = new X_MP_MaintainDetail(Env.getCtx(),req.get_ValueAsInt("MP_MaintainDetail_ID"),null);
			newOT.setMP_Maintain_ID(mainDet.getMP_Maintain_ID());
			mainDetFlag = req.get_ValueAsInt("MP_MaintainDetail_ID");
		}
		newOT.setC_DocType_ID(MDocType.getOfDocBaseType(Env.getCtx(), "OTC")[0].getC_DocType_ID());
		if(!newOT.save())//creada nueva OT
			return false;
		
		if(!createOTTaskDetailMainDet(Job_ID,newOT,mainDetFlag))
			return false;
	...
	{		
		...
		{
			if(OT.get_ValueAsInt("MP_MaintainDetail_ID") > 0)
	...
	public boolean createOTTaskDetailMainDet(int Job_ID, X_MP_OT OT, int ID_MainDet)
	{		
		String sql = "select * from MP_JOBSTANDAR_TASK where MP_JOBSTANDAR_ID=?";
		PreparedStatement pstmt = null;
		try
		{
			...
			while (rs.next())
			{
				...
				ta.setStatus(ta.STATUS_NotStarted);
				if(ID_MainDet > 0)
					ta.set_CustomColumn("MP_MaintainDetail_ID", ID_MainDet);
				ta.saveEx();
