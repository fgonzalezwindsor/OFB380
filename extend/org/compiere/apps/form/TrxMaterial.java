		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					{//faaguilar OFB begin
						lineID = Env.getContextAsInt(Env.getCtx(), m_WindowNo, "PP_Order_ID");
						if (lineID != 0)
						{
							log.fine("PP_Order_ID=" + lineID);
							AD_Window_ID = 53009;//Manufacturing Order
							ColumnName = "PP_Order_ID";
							SQL = "SELECT PP_Order_ID FROM PP_Order WHERE PP_Order_ID=?";
						}
						else
