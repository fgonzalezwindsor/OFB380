...
public class InvoiceGen extends GenForm
{
	...
	public Object 			m_DateFrom = null;//faaguilar
	public Object 			m_DateTo = null;//faaguilar
	public Object  m_User_ID = null; //faaguilar
	public Object  m_POReference = null; //faaguilar
	public Object  m_BPLocation_ID= null; //faaguilar
	...
	private String getOrderSQL()
	{
	    StringBuffer sql = new StringBuffer(
	           ...
	           // + " WHERE i.C_Order_ID=ic.C_Order_ID AND i.DocStatus IN ('IP', 'CO', 'CL')) ");//comment by faaguilar ofb

        if (m_AD_Org_ID != null)
            sql.append(" AND ic.AD_Org_ID=").append(m_AD_Org_ID);
        //ininoles se agrega validacion para que no muestre las ordenes de todas la ORG
		...
		try
		{
			...
			while (rs.next())
			{
				...
				if(OFBForward.NewDescriptionInvoiceGenPA())
				{
					...
					miniTable.setValueAt(new MOrder(Env.getCtx(),rs.getInt(1),null).getDescription(), row, 4);              //  Description faaguuilar Added
