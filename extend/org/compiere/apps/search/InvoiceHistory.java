	...
	implements ActionListener, ChangeListener
{
	...
	private Vector<Vector<Object>> queryProduct ()
	{
		...
		//ininoles se reemplaza sql si existe registro en system configuration
		if(OFBForward.NewSQLBtnHistoryProduct())
		{
			sql = "SELECT h.Name,h.PriceActual,h.PriceList,h.QtyInvoiced,"		//	1,2,3,4
				+ "h.DateInvoiced,h.DocumentNo,"	//	5,6
				+ "h.Name2 as Name,"																//  7
				+ " discount, h.M_PriceList_ID"												//  8,9
				+ " FROM RVOFB_IHistoryQProduct h"
				+ " WHERE C_BPartner_ID=? "
	...
	private Vector<Vector<Object>> queryBPartner ()
	{
		String sql = "SELECT bp.Name,l.PriceActual,l.PriceList,l.QtyInvoiced,"		//	1,2,3,4
			...
			+ "ORDER BY i.DateInvoiced DESC";
		//ininoles se reemplaza sql si existe registro en system configuration
		if(OFBForward.NewSQLBtnHistory())
		{
			sql = "SELECT h.Name,h.PriceActual,h.PriceList,h.QtyInvoiced,"		//	1,2,3,4
				+ "h.DateInvoiced,h.DocumentNo,"	//	5,6
				+ "h.Name2 as Name,"																//  7
				+ " discount, h.M_PriceList_ID"												//  8,9
				+ " FROM RVOFB_IHistoryQBPartner h"
				+ " WHERE M_Product_ID = ? "
				+ " ORDER BY DateInvoiced DESC";
		}
