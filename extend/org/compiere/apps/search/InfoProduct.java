...
public class InfoProduct extends Info implements ActionListener, ChangeListener
{
	...
	public boolean extraProductSearch=false;//faaguilar OFB added
	/**
		...
		boolean multiSelection, String whereClause)
	{
		...
		//
		extraProductSearch="Y".equals(DB.getSQLValueString("LA","select AddProductSearch from " +
				"AD_Client where AD_Client_ID=?",Env.getAD_Client_ID(Env.getCtx()))); //faaguilar OFB added
		statInit();
    ...
    /**faaguilar OFB extrasearch and info begin*/
    MiniTable locatorsTbl = new MiniTable();
    String m_sqllocators;
    private CLabel labelForeign = new CLabel();
	private VComboBox Foreign = new VComboBox();
	private CLabel labelBoxType = new CLabel();
	private VComboBox BoxType = new VComboBox();
	private static final String s_productFromOFB =
		  " m_product_ofb p" 
		+ " LEFT OUTER JOIN M_ProductPrice pr ON (p.M_Product_ID=pr.M_Product_ID AND pr.IsActive='Y')"
		+ " LEFT OUTER JOIN M_AttributeSet pa ON (p.M_AttributeSet_ID=pa.M_AttributeSet_ID)"
		+ " LEFT OUTER JOIN M_Product_PO ppo ON (p.M_Product_ID=ppo.M_Product_ID)"
		+ " LEFT OUTER JOIN C_BPartner bp ON (ppo.C_BPartner_ID=bp.C_BPartner_ID)";
    /**faaguilar OFB extrasearch and info end*/
	...
	private void statInit()
	{
		...
		//faaguilar OFB extra search options
	    Foreign.addItem(" ");
	    Foreign.addItem("Y");
	    Foreign.addItem("N");
	    labelForeign.setText("Origen");
	    labelBoxType.setText("Tipo envase");
	    BoxType.setBackground(AdempierePLAF.getInfoBackground());
		//faaguilar OFB extra search options END
		...
		//faaguilar OFB extra seachr options
		if(extraProductSearch)
		{
		parameterPanel.add(labelBoxType, null);
		parameterPanel.add(BoxType, null);
		parameterPanel.add(labelForeign, null);
		parameterPanel.add(Foreign, null);
		}
		//faaguilar OFB extra seachr options END
        ...
        ColumnInfo[] s_layoutWarehouse = new ColumnInfo[]{
        		...
        		new ColumnInfo(Msg.translate(Env.getCtx(), "QtyAvailable"), "QtyAvailable", Double.class),//faaguilar
        		new ColumnInfo(Msg.translate(Env.getCtx(), "QtyOnHand"), "sum(QtyOnHand)", Double.class),
        		new ColumnInfo(Msg.translate(Env.getCtx(), "QtyReserved"), "QtyReserved", Double.class) //faaguilar
		...
		m_sqlWarehouse += " Group By Warehouse, documentnote,QtyReserved,QtyAvailable "; //faaguilar add QtyReserved QtyAvailable
      ...
      //faaguilar OFB locators begin
        ColumnInfo[] s_layoutLocators = new ColumnInfo[]{
        		new ColumnInfo(Msg.translate(Env.getCtx(), "Warehouse"), "Warehouse", String.class),
        		new ColumnInfo(Msg.translate(Env.getCtx(), "Locator"), "Locator", String.class),
        		new ColumnInfo(Msg.translate(Env.getCtx(), "QtyAvailable"), "0", Double.class),
        		new ColumnInfo(Msg.translate(Env.getCtx(), "QtyOnHand"), "sum(QtyOnHand)", Double.class),
        		new ColumnInfo(Msg.translate(Env.getCtx(), "QtyReserved"), "0", Double.class)};
        /**	From Clause							*/
        s_sqlFrom = " M_PRODUCT_STOCK_V ";
        /** Where Clause						*/
        s_sqlWhere = "M_Product_ID = ?";
        m_sqllocators = locatorsTbl.prepareTable(s_layoutLocators, s_sqlFrom, s_sqlWhere, false, "M_PRODUCT_STOCK_V");
        m_sqllocators += " Group By Warehouse,Locator HAVING sum(QtyOnHand)!=0";
        locatorsTbl.setRowSelectionAllowed(false);
        locatorsTbl.setMultiSelection(false);
        locatorsTbl.addMouseListener(this);
        locatorsTbl.getSelectionModel().addListSelectionListener(this);
        locatorsTbl.autoSize();
        //faaguilar OFB locators end
		...
		//faaguilar OFB locators begin
        jTab.addTab(Msg.translate(Env.getCtx(), "Locator"), new JScrollPane(locatorsTbl));
        //faaguilar OFB locators end
	...
	{
		//	validacion mostrar todas las listas de precios ininoles
		int Order_IDLP = Env.getContextAsInt(Env.getCtx(), p_WindowNo, "C_Order_ID");
		int Invoice_IDLP = Env.getContextAsInt(Env.getCtx(), p_WindowNo, "C_Invoice_ID");
		int pl_IDLP=-1;
		int iv_IDLP=-1;
		if(Order_IDLP>0){
			 pl_IDLP=DB.getSQLValue(null, "select M_PriceList_ID from C_Order where C_Order_ID="+Order_IDLP);
		}
		if(Invoice_IDLP>0){
			iv_IDLP=DB.getSQLValue(null, "select M_PriceList_ID from C_Invoice where C_Invoice_ID="+Invoice_IDLP);
		}
		int plist_id = 0;
		plist_id = Env.getContextAsInt(Env.getCtx(), p_WindowNo, "M_PriceList_ID");
		
		if(pl_IDLP>0)
			M_PriceList_ID=pl_IDLP;
		else if (iv_IDLP>0)
			M_PriceList_ID=iv_IDLP;
		else if (plist_id>0)//ininoles modificacion lista de precios en otras ventanas 
			M_PriceList_ID=plist_id;
		else //ininoles modificacion para que se puedan ver todas las listas de precios, sino se esta parado en un doc
			M_PriceList_ID=0;
		...
		/*else commented by faaguilar OFB
		...
		//
		if(!newView()) //faaguilar OFB begin
			prepareTable(getProductLayout(),
					s_productFrom,
					where.toString(),
				"QtyAvailable DESC, p.Name ASC,Margin DESC");//ininoles se agrega nombre deproducto para el orden
		else //faaguilar OFB end
			prepareTable(getProductLayout(),
					s_productFromOFB,
					where.toString(),
					"QtyAvailable DESC, p.Name ASC, Margin DESC");//ininoles se agrega nombre deproducto para el orden
	    ...
	    Foreign.addActionListener(this);
		boolean noteditplv="Y".equals(DB.getSQLValueString("LA","select readonlyplv from AD_Client where AD_Client_ID=?",Env.getAD_Client_ID(Env.getCtx())));
		if(noteditplv)
	...
	private int findPLV (int M_PriceList_ID)
	{
		...
		/**
		 * faaguilatr OFB
		 * patch blumos version pl*/
		//faaguilar OFB begin
		int Order_ID = Env.getContextAsInt(Env.getCtx(), p_WindowNo, "C_Order_ID");
		int pl_ID=-1;
		if(Order_ID>0){
			 pl_ID=DB.getSQLValue(null, "select M_PriceList_ID from C_Order where C_Order_ID="+Order_ID );
			 log.info("Order_ID :" + Order_ID + " -PL_ID: " + pl_ID);
		}
		if(pl_ID>0)
			M_PriceList_ID=pl_ID;
	...
	protected String getSQLWhere()
	{
		...
		if (!(name.equals("") || name.equals("%")))
			//where.append(" AND UPPER(p.Name) LIKE ?");faaguilar OFB original code comented
			if(!newView() && !alias()) //faaguilar OFB begin
				where.append(" AND (UPPER(p.Name) LIKE ? or UPPER(p.Value) LIKE '%"+name+"%' )");
			else if (alias())
				where.append(" AND (UPPER(p.Name) LIKE ? or UPPER(p.Alias) LIKE '%"+name+"%' )");
			else
				where.append(" AND (UPPER(p.Name) LIKE ? OR UPPER(p.Name2) LIKE '"+name+"' or UPPER(p.Value) LIKE '%"+name+"%' )"); 
		  //faaguilar OFB end
		...
		if(extraProductSearch)
		{
			...
			String Boxt = ((String)BoxType.getSelectedItem());
			if (!(Boxt.equals("")))
				where.append(" AND p.BoxType IN (select value from AD_Ref_List where name like '%"+ Boxt +"%' and ad_reference_id=1000011)");
			log.config("Foreign.getSelectedItem():"+((String)Foreign.getSelectedItem()));	
			if (!((String)Foreign.getSelectedItem()).equals(" "))
				where.append(" AND p.Foreign='"+((String)Foreign.getSelectedItem())+"'");
		}
		//faaguilar OFB extra seachr options END
	...
	protected Info_Column[] getProductLayout()
	{
		...
		if (s_productLayout == null)
		{
			...
			/**faaguilar OFb extra search begin*/
			if("Y".equals(DB.getSQLValueString("LA","select AddProductSearch from AD_Client where AD_Client_ID=?",Env.getAD_Client_ID(Env.getCtx()))))
			{
			list.add(new Info_Column("Envase", "(SELECT Name from AD_ref_list where ad_reference_id=1000011 and value=p.boxtype) as boxType", String.class));
			list.add(new Info_Column("Extranjero", "p.foreign", String.class));
			}
			/**faaguilar OFb extra search end*/
	...
	{
		//	Price List
		String SQL = "SELECT Name from AD_ref_list where ad_reference_id=1000011";
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try
		{
			...
			while (rs.next())
			{
				BoxType.addItem(rs.getString(1));
			}
			DB.close(rs, pstmt);
	...
	}	//	fillBoxType faaguilar OFB
	
	/**
	 *	faaguilar OFB
	 *  verifica la existencia de la vista m_product_ofb
	 */
	public boolean newView()
	{
    	...
    	try
    	{
    		pstmt = DB.prepareStatement("select count(1) from m_product_ofb", null);
    		rs = pstmt.executeQuery();
    		if (rs.next())
    			retValue = true;
	...
	/**faaguilar OFB
	 * verifica la existencias de alias*/
	public boolean alias()
	{
    	...
    	try
    	{
    		pstmt = DB.prepareStatement("select alias from m_product", null);
    		rs = pstmt.executeQuery();
    		if (rs.next())
    			retValue = true;
