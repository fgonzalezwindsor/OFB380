	...
	public ArrayList<Fact> createFacts (MAcctSchema as)
	{
		...
		for (int i = 0; i < p_lines.length; i++)
		{
			...
			if (dr.getAD_Org_ID() != cr.getAD_Org_ID())
			{
				...
				if(OFBProductCost.sameOrg(dr.getAD_Org_ID(), cr.getAD_Org_ID() , as.getCtx()))//faaguilar OFB
					continue;
				//
				...
				/*MCostDetail.createMovement(as, dr.getAD_Org_ID(), 	//	locator org
					...
					description + "(|->)", getTrxName());*///faaguilar OFB original code commented
				OFBProductCost.createMovement(as, dr.getAD_Org_ID(), 	//	locator org
						line.getM_Product_ID(), line.getM_AttributeSetInstance_ID(),
						line.get_ID(), 0,
						costs.negate(), line.getQty().negate(), true,
						description + "(|->)", getTrxName());//faaguilar OFB
				//	Cost Detail To
				/*MCostDetail.createMovement(as, cr.getAD_Org_ID(),	//	locator org 
					...
					description + "(|<-)", getTrxName());*///faaguilar OFB original code commented
				OFBProductCost.createMovement(as, cr.getAD_Org_ID(),	//	locator org 
						line.getM_Product_ID(), line.getM_AttributeSetInstance_ID(),
						line.get_ID(), 0,
						costs, line.getQty(), false,
						description + "(|<-)", getTrxName());//faaguilar OFB
