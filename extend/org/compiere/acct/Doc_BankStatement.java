...
public class Doc_BankStatement extends Doc
{
	...
	private DocLine[] loadLines(MBankStatement bs)
	{
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			//	Set Date Acct
			if (i == 0)
				setDateAcct(bs.getStatementDate());//faaguilar OFB change original code line.getDateAcct()
			docLine.setDateAcct(bs.getStatementDate());//faaguilar OFB
			MPeriod period = MPeriod.get(getCtx(), docLine.getDateAcct(), line.getAD_Org_ID());
	...
	public ArrayList<Fact> createFacts (MAcctSchema as)
	{
		...
		for (int i = 0; i < p_lines.length; i++)
		{
			...
			AD_Org_ID = line.getAD_Org_ID(); //faaguilar OFB
			...
			if ((!as.isPostIfClearingEqual()) && acct_bank_asset.equals(acct_bank_in_transit)) {
				...
				if (amt_stmt_minus_trx.compareTo(Env.ZERO) != 0) {
					...
					/*if(getDateAcct()!=null && fl!=null){//faaguilar OFB begin
						fl.setDateAcct(this.getDateAcct());
						fl.setC_Period_ID(MPeriod.getC_Period_ID(getCtx(), getDateAcct()));
					}*///faaguilar OFB end
				...
				//faaguilar OFB Special Docs Acct begin
				if("K".equals(line.getm_TenderType()) && "Y".equals(line.getm_isReceipt())) //cheques por cobrar
				fl = fact.createLine(line,
						getAccount(Doc.ACCTTYPE_CheckInTransit, as),
						line.getC_Currency_ID(), line.getTrxAmt().negate());
				else if("L".equals(line.getm_TenderType()) && "Y".equals(line.getm_isReceipt()))//letras por cobras
				fl = fact.createLine(line,
						getAccount(Doc.ACCTTYPE_LetrasInTransit, as),
						line.getC_Currency_ID(), line.getTrxAmt().negate());
				else if("K".equals(line.getm_TenderType()) && "N".equals(line.getm_isReceipt()))//cheques por pagar
				fl = fact.createLine(line,
						getAccount(Doc.ACCTTYPE_CheckToPay, as),
						line.getC_Currency_ID(), line.getTrxAmt().negate());
				else if("L".equals(line.getm_TenderType()) && "N".equals(line.getm_isReceipt()))//letras por pagar
				fl = fact.createLine(line,
						getAccount(Doc.ACCTTYPE_LetrasToPay, as),
						line.getC_Currency_ID(), line.getTrxAmt().negate());
				else if("V".equals(line.getm_TenderType()) && "Y".equals(line.getm_isReceipt()))//vale vista x cobrar
				fl = fact.createLine(line,
						getAccount(Doc.ACCTTYPE_VistaInTransit, as),
						line.getC_Currency_ID(), line.getTrxAmt().negate());
				else if("P".equals(line.getm_TenderType()) && "Y".equals(line.getm_isReceipt()))//pagare x cobrar
				fl = fact.createLine(line,
						getAccount(Doc.ACCTTYPE_PagareInTransit, as),
						line.getC_Currency_ID(), line.getTrxAmt().negate());
				else if("V".equals(line.getm_TenderType()) && "N".equals(line.getm_isReceipt()))//vale vista x pagar
				fl = fact.createLine(line,
						getAccount(Doc.ACCTTYPE_VistaToPay, as),
						line.getC_Currency_ID(), line.getTrxAmt().negate());
				else if("P".equals(line.getm_TenderType()) && "N".equals(line.getm_isReceipt()))//pagare x pagar
				fl = fact.createLine(line,
						getAccount(Doc.ACCTTYPE_PagareToPay, as),
						line.getC_Currency_ID(), line.getTrxAmt().negate());
				else
				//faaguilar OFB Special Docs Acct End
				fl = fact.createLine(line,
