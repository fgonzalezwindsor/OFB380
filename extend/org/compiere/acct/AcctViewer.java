...
//		tabbedPane.add(graphPanel,   Msg.getMsg(Env.getCtx(), "ViewerGraph"));
	...
	{
		//faaguilar OFB begin
		if(m_data.AD_Table_ID==335 && new MPayment (Env.getCtx(),  m_data.Record_ID, null).get_ValueAsBoolean("IsCustomAcct"))
		{
				actionQuery();
		}
		//validacion para solicitud d epago
		else if(m_data.AD_Table_ID==1000043 && new X_C_PaymentRequest(Env.getCtx(),  m_data.Record_ID, null).get_ValueAsBoolean("IsCustomAcct"))
		{
				actionQuery();
		}
		//validacion para resolucion de asignación
		else if(m_data.AD_Table_ID==1000049 && new X_DM_Document(Env.getCtx(),  m_data.Record_ID, null).get_ValueAsBoolean("IsCustomAcct"))
		{
				actionQuery();
		}
		//validacion para diario de cg
		else if(m_data.AD_Table_ID==224 && new MJournal(Env.getCtx(),  m_data.Record_ID, null).get_ValueAsBoolean("IsCustomAcct"))
		{
				actionQuery();
		}
		else
