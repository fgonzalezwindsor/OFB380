		...
		if (getDocumentType().equals(DOCTYPE_GLJournal))
		{
			...
			for (int i = 0; i < p_lines.length; i++)
			{
				if (p_lines[i].getC_AcctSchema_ID () == as.getC_AcctSchema_ID ())
				{
					...
					//faaguilar OFB begin product cost
					
					MJournalLine jline= new MJournalLine(getCtx(),p_lines[i].get_ID(),getTrxName());
					if( jline.get_Value("M_Product_ID") !=null && ((Integer)jline.get_Value("M_Product_ID")).intValue()>0){	
						OFBProductCost.createJournal(as, jline.getAD_Org_ID(), 
								((Integer)jline.get_Value("M_Product_ID")).intValue(), 0,
								jline.getGL_JournalLine_ID() , 0,
								jline.getAmtAcctDr().signum()>0?jline.getAmtAcctDr():jline.getAmtAcctCr().negate(), jline.getQty(), 		//	Qty
								"Costo Inicial", getTrxName());
					}
					//faaguilar OFB end product cost
				}
