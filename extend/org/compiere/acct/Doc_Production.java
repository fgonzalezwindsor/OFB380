...
public class Doc_Production extends Doc
{
	...
	public ArrayList<Fact> createFacts (MAcctSchema as)
	{
		...
		for (int i = 0; i < p_lines.length; i++)
		{
			...
			{	
				if (line.isProductionBOM())
				{
					...
					for (int ii = 0; ii < p_lines.length; ii++)
					{
						...
						{
							BigDecimal ppCost= line0.getProductCosts(as, line.getAD_Org_ID(), false);
							log.config("ppCost:"+ppCost);
							bomCost = bomCost.add(ppCost);
						}
						
						log.config("*bomCost:"+bomCost);
					}
				    ...
				    if (line.getQty() != mpp.getProductionQty()) {
				    	...
				    	log.config("1Cost :"+costs + " line.getQty():"+line.getQty());
				    	costs = costs.divide(mpp.getProductionQty(), as.getCostingPrecision(), BigDecimal.ROUND_HALF_UP);
				    	log.config("2Cost :"+costs+" mpp.getProductionQty():"+mpp.getProductionQty());
				    }
				}
				else{
					log.config("*isNotProductionBOM");
					costs = line.getProductCosts(as, line.getAD_Org_ID(), false);
				}
			}
			// end MZ
			log.config("Cost :"+costs);
			//  Inventory       DR      CR
			...
			if (line.isProductionBOM())
				description += "(*)";
			//ininoles se cambia por nueva clase de generacion de costos NO
			MCostDetail.createProduction(as, line.getAD_Org_ID(), 
				line.getM_Product_ID(), line.getM_AttributeSetInstance_ID(), 
				line.get_ID(), 0, 
				costs, line.getQty(), 
				description, getTrxName());
			/*OFBProductCost.createProduction(as, line.getAD_Org_ID(), 
					line.getM_Product_ID(), line.getM_AttributeSetInstance_ID(), 
					line.get_ID(), 0, 
					costs, line.getQty(), 
					description, getTrxName());*/			
