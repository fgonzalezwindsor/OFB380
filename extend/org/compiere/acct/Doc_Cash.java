...
public class Doc_Cash extends Doc
{
	...
	public ArrayList<Fact> createFacts (MAcctSchema as)
	{
		...
		//faaguilar OFB begin
		MCash cash = (MCash)getPO();
		if(cash.getDocStatus().equals("VO"))
			return facts;
	    //faaguilar OFB end
		...
		for (int i = 0; i < p_lines.length; i++)
		{
			...
			{   //  amount is pos/neg
				...
				{
					MPayment pp = new MPayment(getCtx(), line.getC_Payment_ID(),getTrxName());
					if(!pp.getTrxType().equals("X"))
					{
						if(getAccount(line.getC_Payment_ID())==null)
						{	p_Error = "Payment not posted yet";
							return null;
						}
						
						
						fact.createLine(line,
								getAccount(line.getC_Payment_ID()),
								line.getC_Currency_ID(), line.getAmount().negate());
					}
					else
						fact.createLine(line,
	 ...
	 * faaguilar OFB
	 * custom method*/
	public MAccount getAccount (int payment_ID)
		{
			
		    MPayment pay = new MPayment(getCtx(),payment_ID,getTrxName());
		    String sql="";
		    if(pay.isReceipt())
		    	sql = "SELECT * "
						+ "FROM Fact_Acct "
