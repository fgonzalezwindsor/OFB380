...
public class Doc_AllocationHdr extends Doc
{
	...
	public ArrayList<Fact> createFacts (MAcctSchema as)
	{
		...
		for (int i = 0; i < p_lines.length; i++)
		{
			...
			if (invoice == null)
			{
				...
				{
					if(line.getWriteOffAmt().signum()==0)//faaguilar OFB
			...
			else if (invoice.isSOTrx())
			{
				...
				if (Env.ZERO.compareTo(line.getWriteOffAmt()) != 0)
				{
					...
					if (fl != null && payment != null)
						fl.setAD_Org_ID(payment.getAD_Org_ID());
					log.config(" Write off		DR:" + fl );
				}
						
				//	AR Invoice Amount	CR
				if (as.isAccrual())
				{
					//faaguilar OFB Special Docs begin
					if (invoice.getDocBase().equals("ARB"))
						bpAcct = getAccount(Doc.ACCTTYPE_ARB, as);
					else if (invoice.getDocBase().equals("PTK"))
					{
						if(invoice.getTenderType().equals("K"))
						bpAcct = getAccount(Doc.ACCTTYPE_PTCheck, as);
						else
						bpAcct = getAccount(Doc.ACCTTYPE_PTLetra, as);
					}
					else if (invoice.getDocBase().equals("FAT"))
					{
						bpAcct = getAccount(Doc.ACCTTYPE_FAT, as);
					}
					else
						bpAcct = getAccount(Doc.ACCTTYPE_C_Receivable, as);
					//faaguilar OFB Special Docs end
					
					//faaguilar OFB  begin 
					if(allocationSource.compareTo(Env.ZERO)<0)
						fl = fact.createLine (line, bpAcct,
								getC_Currency_ID() , allocationSource.abs(), null);
					else
					//faaguilar OFB  end
					fl = fact.createLine (line, bpAcct,
							getC_Currency_ID(), null, allocationSource.abs());		//	payment currency 
					
					log.config("AR Invoice Amount	CR:" + fl );
			...
			else
			{
				...
				if (as.isAccrual())
				{
					...
					//faaguilar OFB begin
					if (invoice.getDocBase().equals("APB")) 
						bpAcct = getAccount(Doc.ACCTTYPE_APB, as); 
					
					if(allocationSource.compareTo(Env.ZERO)<0) 
						fl = fact.createLine (line, bpAcct, 
								getC_Currency_ID(), null, allocationSource.abs());
					else if(invoice.getC_DocType().getDocBaseType().equals(Doc.DOCTYPE_API_Compromise))//faaguilar compromisos
					{
						
						if(payment.get_ValueAsInt("C_InvoicePaySchedule_ID")>0)//cuota
						{
							MInvoicePaySchedule is = new MInvoicePaySchedule(Env.getCtx(),payment.get_ValueAsInt("C_InvoicePaySchedule_ID"),getTrxName());
							int months = monthDiff (invoice.getDateInvoiced(), is.getDueDate());
							/*BigDecimal uf = (BigDecimal)is.get_Value("ForeignPrice");
							if(uf!=null)
							{
								BigDecimal amt = MConversionRate.convertBase(Env.getCtx(), uf,	//	CM adjusted
										1000000, getDateDoc(), 114, invoice.getAD_Client_ID(), invoice.getAD_Org_ID());
							}*/
							if(allocationSource.abs().floatValue()!=is.getDueAmt().floatValue())//diferencia de cambio
							{
								log.config("Doferencia "+ allocationSource.abs().floatValue() +" : "+ is.getDueAmt().floatValue());
								fl = fact.createLine (line, as.getCurrencyBalancing_Acct(), 
										getC_Currency_ID(), is.getDueAmt().subtract(allocationSource.abs()) );
								
							}
							if(months>12)//largo
							{
								log.config("largo");
								fl = fact.createLine (line, getAccount(Doc.ACCTTYPE_APALargo, as, invoice.getC_DocType_ID()), 
										getC_Currency_ID(), null,allocationSource.abs() );
								
								fl = fact.createLine (line, getAccount(Doc.ACCTTYPE_APACorto, as, invoice.getC_DocType_ID()), 
										getC_Currency_ID(), allocationSource.abs(),null );
							}
							else
							{
					...
					fl = fact.createLine (line, bpAcct,
							getC_Currency_ID(), allocationSource.abs(), null);		//	payment currency
					if (fl != null)
						allocationAccounted = fl.getAcctBalance();
					if (fl != null && invoice != null)
						fl.setAD_Org_ID(invoice.getAD_Org_ID());
					
					log.config("AP Invoice Amount	DR:" + fl );
				...
				else if (isUsingClearing && line.getC_CashLine_ID() != 0) // Avoid usage of clearing accounts
				{
					...
					log.config("Avoid usage of clearing accounts:" + fl );
				}
			}
	 ...
	 *  @return Account
	 */
	private final MAccount getAccount (int AcctType, MAcctSchema as, int Doctype_ID)
	{
		int C_ValidCombination_ID = getValidCombination_ID(AcctType, as, Doctype_ID);
	 ...
	 */
	private int getValidCombination_ID (int AcctType, MAcctSchema as, int Doctype_ID)
	{
	...
	//  Get Acct
			...
			if (Account_ID == 0)
			{
				log.severe ("NO account Type="
					+ AcctType + ", Doctype_ID=" +  Doctype_ID);
				return 0;
			}
			return Account_ID;
	}
	
	private int monthDiff (Timestamp start, Timestamp end)
	{
		Calendar firstDate = new GregorianCalendar();
		Calendar secondDate = new GregorianCalendar();   
		firstDate.setTimeInMillis(start.getTime());
		secondDate.setTimeInMillis(end.getTime());
