...
//		MMatchInv matchInv = (MMatchInv)getPO();
		...
		X_C_DocType DocType= MDocType.getOfDocBaseType(getCtx(), DOCTYPE_MatMatchInv)[0];
		...
		**/
		//faaguilar OFB begin
		BigDecimal invAmt = getLineAmt (MInvoice.Table_ID, m_invoiceLine.getC_Invoice_ID(), m_invoiceLine.getC_InvoiceLine_ID(), as.getC_AcctSchema_ID());
		BigDecimal inoutAmt = getLineAmt (MInOut.Table_ID, m_receiptLine.getM_InOut_ID(), m_receiptLine.getM_InOutLine_ID(), as.getC_AcctSchema_ID());
		//ininoles verificacion de si existe contabilidad
		int invQty = getQtyAmt(MInvoice.Table_ID, m_invoiceLine.getC_Invoice_ID(), m_invoiceLine.getC_InvoiceLine_ID(), as.getC_AcctSchema_ID());
		int inoutQty = getQtyAmt(MInOut.Table_ID, m_receiptLine.getM_InOut_ID(), m_receiptLine.getM_InOutLine_ID(), as.getC_AcctSchema_ID()); 
		Boolean costoZero = true;
		
		log.config("invAmt:"+invAmt);
		log.config("inoutAmt:"+inoutAmt);
		
		log.config("invQty:"+invQty);
		log.config("inoutQty:"+inoutQty);
		...
		FactLine dr = fact.createLine (null,
			...
			as.getC_Currency_ID(), inoutAmt, null);			// updated below
		if (dr == null  && costoZero == false)
		{
	...
	//  InventoryClearing               CR
		...
		cr = fact.createLine (null, expense,
				as.getC_Currency_ID(), null, invAmt);		//	updated below
		cr.setQty(getQty().negate());
		
		BigDecimal realAmt= invAmt.subtract(inoutAmt);
		log.config("realAmt:"+realAmt);
		if(realAmt.signum()!=0)
			{
				
				BigDecimal pricein = inoutAmt.divide(m_receiptLine.getQtyEntered(), 2, BigDecimal.ROUND_HALF_UP);
				BigDecimal priceinv = invAmt.divide(m_invoiceLine.getQtyInvoiced(), 2, BigDecimal.ROUND_HALF_UP);
				log.info("price in:" + pricein);
				log.info("price inv:" + priceinv);
				
				if(pricein.compareTo(priceinv)!=0)// distinto precio
				{
					BigDecimal diff =priceinv.subtract(pricein);
					diff = diff.multiply(m_receiptLine.getQtyEntered());
					
					
					
					OFBProductCost.createInvoice(as, m_invoiceLine.getAD_Org_ID(), 
							m_invoiceLine.getM_Product_ID(), m_invoiceLine.getM_AttributeSetInstance_ID(),
							m_invoiceLine.getC_InvoiceLine_ID(), 0,diff
							, Env.ZERO, 		//	Qty
							"Ajuste Diferencia de Cambio", getTrxName());
