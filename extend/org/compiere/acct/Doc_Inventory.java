...
//		END faaguilar validation not post end ininoles
		...
		for (int i = 0; i < p_lines.length; i++)
		{
			...
			//BigDecimal costs = line.getProductCosts(as, line.getAD_Org_ID(), true, "M_InventoryLine_ID=?");//faaguilar OFB original code commented
			BigDecimal costs = OFBProductCost.getProductCosts(as, "M_InventoryLine_ID=? ", line,as.getC_AcctSchema_ID(), getTrxName(), getCtx());//faaguilar OFB
			// end MZ
			...
			log.config("costs antes del if por reverso "+costs);
			if (m_DocStatus.equals(MInventory.DOCSTATUS_Reversed) && m_Reversal_ID !=0 && line.getReversalLine_ID() != 0)
			{	
				log.config("costs antes de pregunta validacion:"+costs);
				...
				log.config("costs despues de pregunta validacion:"+costs);
				costs = cr.getAcctBalance(); //get original cost
				//ininoles si cantidad es negativa volvemos a negar (consumos en negativo reversados)
			...
			//faaguilar OFB begin
//			Cost Detail
			log.config("costs antes del abs "+costs);
			log.config("cantidad antes del abs "+line.getQty());
			if(line.getQty().signum()>0)
				costs=costs.abs();
			log.config("costs despues del abs "+costs);
