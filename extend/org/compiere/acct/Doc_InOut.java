	...
	public ArrayList<Fact> createFacts (MAcctSchema as)
	{
		...
		if (getDocumentType().equals(DOCTYPE_MatShipment) && isSOTrx())
		{
			for (int i = 0; i < p_lines.length; i++)
			{
				...
				//BigDecimal costs = line.getProductCosts(as, line.getAD_Org_ID(), true, "M_InOutLine_ID=?"); faaguilar OFB original code commented
				BigDecimal costs = OFBProductCost.getProductCosts(as, "M_InOutLine_ID=? ", line, as.getC_AcctSchema_ID(),getTrxName(), getCtx());//faaguilar OFB
				// end MZ
				...
				//  CoGS            DR
				if(line.getQty().signum()<0)//entrega
					dr = fact.createLine(line,
						line.getAccount(ProductCost.ACCTTYPE_P_Cogs, as),
						as.getC_Currency_ID(), costs.abs(), null);
				else//reversa
					dr = fact.createLine(line,
							line.getAccount(ProductCost.ACCTTYPE_P_Cogs, as),
							as.getC_Currency_ID(),null , costs.abs());
				...
				dr.setAD_Org_ID(line.getAD_Org_ID());//faaguilar OFB
				...
				cr.setAD_Org_ID(line.getAD_Org_ID());//faaguilar OFB
				if (m_DocStatus.equals(MInOut.DOCSTATUS_Reversed) 
						&& m_Reversal_ID !=0 && line.getReversalLine_ID() != 0)
				{
				...
				if (line.getM_Product_ID() != 0)
				{
					/*MCostDetail.createShipment(as, line.getAD_Org_ID(), 
						...
						line.get_ID(), 0,
						costs, line.getQty(),
						line.getDescription(), true, getTrxName());*///faaguilar OFB original code commented
					
					OFBProductCost.createShipment(as, line.getAD_Org_ID(), 
							line.getM_Product_ID(), line.getM_AttributeSetInstance_ID(),
							line.get_ID(), 0,
							costs, line.getQty(),
							line.getDescription(), true, getTrxName());//faaguilar OFB costo negativo al entregar
				}
			}	//	for all lines
		...
		else if ( getDocumentType().equals(DOCTYPE_MatReceipt) && isSOTrx() )
		{
			for (int i = 0; i < p_lines.length; i++)
			{
				...
				// MZ Goodwill
				// if Shipment CostDetail exist then get Cost from Cost Detail 
				//BigDecimal costs = line.getProductCosts(as, line.getAD_Org_ID(), true, "M_InOutLine_ID=?");faaguilar OFB commented original code
				BigDecimal costs = OFBProductCost.getProductCosts(as, "M_InOutLine_ID=? ", line,as.getC_AcctSchema_ID(), getTrxName(), getCtx());//faaguilar OFB
				// end MZ
				...
				if (line.getM_Product_ID() != 0)
				{
					...
					OFBProductCost.createShipment(as, line.getAD_Org_ID(), 
							...
							line.get_ID(), 0,
							costs, line.getQty(),
							line.getDescription(), true, getTrxName());//faaguilar OFB costo positivo al devolver
				}
		...
		else if (getDocumentType().equals(DOCTYPE_MatReceipt) && !isSOTrx())
		{
			for (int i = 0; i < p_lines.length; i++)
			{
				...
				DocLine line = p_lines[i];
				Timestamp dateCost= line.getDateAcct();//faaguilar OFB
				BigDecimal costs = null;
				//BigDecimal costsAcct = null;
				MProduct product = line.getProduct();
				...
				{*/
					...
					if (C_OrderLine_ID > 0) 
					{
					    ...
					    dateCost = orderLine.getDateOrdered();//faaguilar OFB
					    //
					    ...
					    /*if(C_Currency_ID !=as.getC_Currency_ID())//faaguilar OFB
					    {
					    	costs = MConversionRate.convert(getCtx(), costs, C_Currency_ID, as.getC_Currency_ID(), dateCost, 0, getAD_Client_ID(), line.getAD_Org_ID());
					    	costs = MConversionRate.convert(getCtx(), costs,as.getC_Currency_ID() ,C_Currency_ID , getDateAcct(), 0, getAD_Client_ID(), line.getAD_Org_ID());
					    }*/// ya no usar
                    }
                    else
                    {
                        //costs = line.getProductCosts(as, line.getAD_Org_ID(), false);	//	current costs faaguilar OFB original code commented
                        costs = OFBProductCost.getProductCosts(as, "M_InOutLine_ID=? ", line,as.getC_AcctSchema_ID(), getTrxName(), getCtx());//faaguilar OFB
                    }
                    //
				/*faaguilar OFB codigo comentado
				 * }
				...
				//faaguilar precio 0 begin
				boolean precioZero=false;
				if(C_OrderLine_ID>0 && (costs == null || costs.signum() == 0))
					precioZero=true;
				//faaguilar precio 0 end
				
				if ((costs == null || costs.signum() == 0) && !precioZero )
				{
					 costs = OFBProductCost.getProductCosts(as, "M_InOutLine_ID=? ", line,as.getC_AcctSchema_ID(), getTrxName(), getCtx());//faaguilar OFB
					 if (costs == null || costs.signum() == 0)
						{
				...
				/*dr = fact.createLine(line, assets,
					as.getC_Currency_ID(), costs, null);*/
				if(line.getQty().signum()>0)//recibo
					dr = fact.createLine(line, assets,
						C_Currency_ID, costs.abs(), null);
				else//reverso
					dr = fact.createLine(line, assets,
							C_Currency_ID, null, costs.abs());
				...
				if (line.getM_Product_ID() != 0)
				{
					...
					OFBProductCost.createShipment(as, line.getAD_Org_ID(), 
							...
							line.get_ID(), 0,
							costs, line.getQty(),
							line.getDescription(), false, getTrxName());//faaguilar OFB
				}
				//faaguilar OFB costs para el recibo de provedor end
			}
		}	//	Receipt
         //	  *** Purchasing - return
			...
			for (int i = 0; i < p_lines.length; i++)
			{
				...
				*/
				...
				if (C_OrderLine_ID > 0) 
				{
					...
					if (costs == null || costs.signum() == 0)
						costs = orderLine.getPriceActual();
					costs = costs.multiply(line.getQty().negate());
				}
				else
				{
					//costs = line.getProductCosts(as, line.getAD_Org_ID(), false);	//	current costs
				...
				//faaguilar OFB costs para el devolucion de provedor begin
				if (line.getM_Product_ID() != 0)
				{	
					...
					OFBProductCost.createShipment(as, line.getAD_Org_ID(), 
							line.getM_Product_ID(), line.getM_AttributeSetInstance_ID(),
							line.get_ID(), 0,
							//costs.negate(), line.getQty().negate(),
							costs.negate(), line.getQty(), //se deja de negar cantidad ya que se niega en el load lines
							line.getDescription(), false, getTrxName());//faaguilar OFB
				}
				//faaguilar OFB costs para el devolucion de provedor end
