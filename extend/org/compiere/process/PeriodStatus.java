...
public class PeriodStatus extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		...
		if(!preRequitos(period))//faaguilar begin
		{
	...
	{
		if(p_PeriodAction.equals(MPeriodControl.PERIODACTION_OpenPeriod))
		{
			int i = DB.getSQLValue(get_TrxName(), "Select COUNT(1) "+
			"from C_Period p "+
			"Inner Join C_PeriodControl pc on (p.C_Period_ID=pc.C_Period_ID) "+
			"where C_Year_ID=? and PeriodNo>? and pc.periodstatus='C' ", p.getC_Year_ID(),p.getPeriodNo());
			
			if(i>0)
				return false;
		}
		if(p_PeriodAction.equals(MPeriodControl.PERIODACTION_ClosePeriod))
		{
			int i = DB.getSQLValue(get_TrxName(), "Select COUNT(1) "+
			"from C_Period p "+
			"Inner Join C_PeriodControl pc on (p.C_Period_ID=pc.C_Period_ID) "+
			"where C_Year_ID=? and PeriodNo<? and pc.periodstatus='O' ", p.getC_Year_ID(),p.getPeriodNo());
