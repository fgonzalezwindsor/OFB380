...
public class DocumentEngine implements DocAction
{
	...
	public boolean processIt (String action)
	{
		...
		/**
		 * faaguilar OFB
		 * comentado codigo original y agregado codigo de manejo de estado
		 * en proceso y esperando confirmacion*/
//		if (ACTION_Prepare.equals(m_action))
//			return STATUS_InProgress.equals(prepareIt());
		if (ACTION_Prepare.equals(m_action))
		{
			
			String mystatus=prepareIt(); 
			if(mystatus.equals(STATUS_InProgress)|| mystatus.equals(STATUS_WaitingConfirmation))
				return true;
			else
				return false;
		}
...
//		Locked
		...
		if (AD_Table_ID == MOrder.Table_ID)
		{
			...
			/**
			 * faaguilar OFB 
			 * control de acciones para el estado Waiting confirmation de ordenes de compra*/
			else if (docStatus.equals(DocumentEngine.STATUS_WaitingConfirmation))//faaguilar OFB begin
			{
				options[index++] = DocumentEngine.ACTION_Complete; //faaguilar OFB end
			}
		}
		/********************
		 *  Shipment
		...
		{
			if (docStatus.equals(DocumentEngine.STATUS_Drafted))//faaguilar
				options[index++] = DocumentEngine.ACTION_Prepare;//faaguilar
		...
		else if (AD_Table_ID == MPayment.Table_ID)
		{
			...
			if (docStatus.equals(DocumentEngine.STATUS_Drafted))//faaguilar OFB
				options[index++] = DocumentEngine.ACTION_Prepare;//faaguilar OFB
		}
		/********************
		 *  GL Journal
		 */
		...
		/**
		 * faaguilar OFB
		 * agregado manejo de acciones para las requisition*/
		//faaguilar OFB requisitions special begin
		else if (AD_Table_ID ==MRequisition.Table_ID)
		{
			...
			if (docStatus.equals(DocumentEngine.ACTION_ReActivate))
			{
				options[index++] = DocumentEngine.STATUS_Completed;
			}
			
			if (docStatus.equals(DocumentEngine.STATUS_WaitingConfirmation))
			{
				options[index++] = DocumentEngine.STATUS_Completed;
			}
			
		}//END faaguilar OFB requisitions special end
		return index;
	}
