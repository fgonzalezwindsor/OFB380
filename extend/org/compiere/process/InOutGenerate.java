...
public class InOutGenerate extends SvrProcess
{
			...
			while (rs.next ())		//	Order
			{
				...
				for (int i = 0; i < lines.length; i++)
				{
					...
					MStorage[] storages = getStorages(line.getM_Warehouse_ID(),
							 ...
							 null, MClient.MMPOLICY_FiFo.equals(MMPolicy));//faaguilar OFB minGuaranteeDate by null
		...
		MStorage[] storages, boolean force)
	{
		...
		if (m_shipment == null)
		{
			...
			//ininoles agregar OFBForguard para que fecha sea sacada desde la orden
			if(OFBForward.DateSalesDocumentFromOrderPA())
				m_movementDate = order.getDatePromised();
			m_shipment = new MInOut (order, order.getC_DocType().getC_DocTypeShipment_ID(), m_movementDate);//faaguilar OFB
		...
		//faaguilar OFB fix2 priority
		MProduct product = orderLine.getProduct();
		String MMPolicy = product.getMMPolicy();
		MClient client = MClient.get(getCtx());
		storages=null;
		storages=getStorages(orderLine.getM_Warehouse_ID(), 
						orderLine.getM_Product_ID(),
						orderLine.getM_AttributeSetInstance_ID(), null, //faaguilar OFB minGuaranteeDate by null
