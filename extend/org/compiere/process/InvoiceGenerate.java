	...
	private String generate (PreparedStatement pstmt)
	{
		try
		{
			...
			while (rs.next ())
			{
				...
				//ininoles que no separe las ordenes mutual
				MClient clientVal = new MClient(getCtx(), get_TrxName());
				if (!p_ConsolidateDocument 
					|| (m_invoice != null 
					&& m_invoice.getC_BPartner_Location_ID() != order.getBill_Location_ID()
					&& !clientVal.getName().toLowerCase().contains("mutual")))
					completeInvoice();
		...
		BigDecimal qtyInvoiced, BigDecimal qtyEntered)
	{
		if (m_invoice == null)
		{
			if(OFBForward.DateSalesDocumentFromOrderPA())
				p_DateInvoiced = order.getDatePromised();
