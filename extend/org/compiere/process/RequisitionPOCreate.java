...
public class RequisitionPOCreate extends SvrProcess
{
	...
	private BigDecimal  m_Price = Env.ZERO;
	...
	private void process (MRequisitionLine rLine) throws Exception
	{
		...
		if (m_orderLine == null
			...
			|| rLine.getDescription()!= m_Description // se agregan validaciones para descripcion(no agrupar) faaguilar 
			|| rLine.getPriceActual() != m_Price //se agrega validacion para precio (no agrupar) faaguilar
			)
		{
			newLine(rLine);
	...
	private void newOrder(MRequisitionLine rLine, int C_BPartner_ID) throws Exception
	{
		...
		if (m_order == null)
		{
			...
			m_order.setM_Warehouse_ID(rLine.getParent().getM_Warehouse_ID());//faaguilar OFB
			//	Prepare Save
			m_order.saveEx();
			// Put to cache
	...
	private void newLine(MRequisitionLine rLine) throws Exception
	{
		...
		if (product != null)
		{
			...
			m_orderLine.setPrice(rLine.getPriceActual());// ininoles faaguilar
		}
		else
		{
		...
		m_orderLine.setDescription(rLine.getDescription());	//OFB Ininoles faaguilar
		//como siempre debe crear una linea se copia la descripcion tambien.		
