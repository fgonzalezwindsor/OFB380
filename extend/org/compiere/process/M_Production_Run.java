					...
					{	
						...
						for (X_M_ProductionLine pline : production_lines)
						{
							...
							else if(MovementQty.signum() < 0)
							{
								...
								//ininoles no validara productos tipo servicio. 
								MProduct prodTemp = new MProduct(getCtx(), pline.getM_Product_ID(), get_TrxName());								
								if(mustBeStocked && QtyAvailable.add(MovementQty).signum() < 0 && prodTemp.getProductType().compareTo("S") != 0 )//ininoles end
								{	
