...
public class CreateInvoicedAsset extends SvrProcess
{
	...
	protected String doIt() throws java.lang.Exception
	{
		...
		String sql =" SELECT * FROM C_InvoiceLine WHERE A_Processed='N' and AD_Client_ID = ?" 
			+ " and A_CreateAsset = 'Y' and Processed = 'Y' And QtyInvoiced>0"; //faaguilar OFB add And QtyInvoiced>0"
		...
		try {
			...
			while (rs.next()){
				...
				int cant = rs.getInt("QtyInvoiced");  //faaguilar OFB
				for(int j=1; j<=cant; j++){			//faaguilar OFB
				...
				if (rs.getString("A_CapvsExp").equals("Cap")){
					...
					if (assetgrpacct.isActive()){//faaguilar OFB					
					    ...
					    {
							asset.setAD_Org_ID(rs.getInt("AD_Org_ID")); //added by faaguilar ofb
							asset.setAssetServiceDate(new Timestamp(TimeUtil.getToday().getTimeInMillis())); //added by faaguilar ofb
							...
							asset.setA_QTY_Current(Env.ONE);//faaguilar OFB
							asset.setA_QTY_Original(Env.ONE);//faaguilar OFB
							asset.save();
							asset.setA_Parent_Asset_ID(asset.getA_Asset_ID());
							asset.save();
							logOFB="asset.save();";
							
							boolean isdepreciate = assetgrpacct.isActive();
							
							if (isdepreciate)
							{
								
								log.config("if (isdepreciate)");
								assetacct.setAD_Org_ID(rs.getInt("Ad_Org_ID"));//added by faaguilar ofb
								...
								log.config("OFB new accounting Begin");
							    ...
							    //faaguilar OFB begin
							    change.setAssetValueAmt(InvoiceLine.getLineNetAmt().divide(InvoiceLine.getQtyEntered(), BigDecimal.ROUND_HALF_EVEN));
							    change.setA_QTY_Current(Env.ONE);
							    change.setA_QTY_Original(Env.ONE);
							    //faaguilar OFB begin
							    change.save();
							    log.config("change.save();");
							...
							//faaguilar OFB begin
							assetwk.setA_Asset_Cost(assetwk.getA_Asset_Cost().add(InvoiceLine.getLineNetAmt().divide(InvoiceLine.getQtyEntered(), BigDecimal.ROUND_HALF_EVEN)));							
						...
						{		
							...
							try {
								...
								while (rs3.next()){	
									...
									assetwk.setA_QTY_Current(InvoiceLine.getQtyEntered());	
									//faaguilar OFB begin
									assetwk.setA_Asset_Cost(assetwk.getA_Asset_Cost().add(InvoiceLine.getLineTotalAmt().divide(InvoiceLine.getQtyEntered(), BigDecimal.ROUND_HALF_EVEN)));							
									assetwk.setA_QTY_Current(Env.ONE);
									//faaguilar OFB end
									assetwk.save();
									...
									//faaguilar OFB begin
									assetadd.setA_QTY_Current(Env.ONE);
									assetadd.setAssetValueAmt(InvoiceLine.getLineTotalAmt().divide(InvoiceLine.getQtyEntered(), BigDecimal.ROUND_HALF_EVEN));
									//faaguilar OFB end
									assetadd.save();
						            ...
						            change.setA_QTY_Current(assetadd.getA_QTY_Current());  
						            //faaguilar OFB begin
						            change.setAssetValueAmt(InvoiceLine.getLineTotalAmt().divide(InvoiceLine.getQtyEntered(), BigDecimal.ROUND_HALF_EVEN));
								    change.setA_QTY_Current(Env.ONE);
								    change.setA_QTY_Original(Env.ONE);
						            //faaguilar OFB end
						            change.save();
