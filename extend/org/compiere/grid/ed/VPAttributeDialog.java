	...
	implements ActionListener
{
		...
		boolean productWindow, int AD_Column_ID, int WindowNo)
	{
		...
		try{
			Boolean editable = "true".equals(role.get_ValueAsString("EditAttribute"));
			if(!editable){
	...
	//	Lot
	private VString fieldLotString = new VString ("Lot", false, false, true, 20, 40, null, null);//faaguilar OFB largo 40
	...
	private boolean initAttributes ()
	{
		...
		//faaguilar begin
		if(as.get_ValueAsBoolean("IsDynamic"))
		  dinamico = true;
		//faaguilar end
		...
		if (m_productWindow)
		{
			...
			for (int i = 0; i < attributes.length; i++)
				addAttributeLine (attributes[i], true, !m_productWindow,i);
		}
		else	//	Set Instance Attributes
		{
			...
			for (int i = 0; i < attributes.length; i++)
				addAttributeLine (attributes[i], false, false,i);
		}
	 ...
	 */
	private void addAttributeLine (MAttribute attribute, boolean product, boolean readOnly ,int x)
	{
		...
		if (MAttribute.ATTRIBUTEVALUETYPE_List.equals(attribute.getAttributeValueType()))
		{
			...
			if(dinamico && x==1)//faaguilar begin
			{
				attrtemp = attribute;
				
				if(instance == null)
					editor = new CComboBox();
				else
				{
					values = attrtemp.getMAttributeValues( ((MAttributeValue)((CComboBox)m_editors.get(0)).getValue()).getName() );
					editor = new CComboBox(values);
				}
			...
			if(dinamico && x==0)//faaguilar
				editor.addActionListener(this);//faaguilar
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		else if(e.getSource().equals(m_editors.get(0))) //faaguilar
		{
			CComboBox editor_principal = (CComboBox)m_editors.get(0);
			
			MAttributeValue[] values = attrtemp.getMAttributeValues( ((MAttributeValue)editor_principal.getValue()).getName() );	//	optional = null
			
			DefaultComboBoxModel editor_3 = (DefaultComboBoxModel)((CComboBox) centerPanel.getComponent(5)).getModel();
			editor_3 = new DefaultComboBoxModel(values);
			((CComboBox)m_editors.get(1)).setModel(editor_3);
	...
	private boolean cmd_select()
	{
		...
		ResultSet rs = null;
		log.config("SQL:" + sql);
		log.config("M_Product_ID ="+m_M_Product_ID);
		try
		{
