	...
	implements ActionListener
{
	...
	private CLabel kBankAccountLabel = new CLabel();//faaguilar OFB
	private CComboBox kBankAccountCombo = new CComboBox();//faaguilar OFB
	private CLabel tBankAccountLabel = new CLabel();//faaguilar OFB
	private CComboBox tBankAccountCombo = new CComboBox();//faaguilar OFB
	...
	private void jbInit() throws Exception
	{
		...
		//	,GridBagConstraints.EAST, GridBagConstraints.NONE, new Insets(5, 5, 5, 5), 0, 0)); faaguilar OFB original comment
		
		//faaguilar OFB begin
		kBankAccountLabel.setText(Msg.translate(Env.getCtx(), "C_BankAccount_ID"));
		kPanel.add(kBankAccountLabel, new GridBagConstraints(0, 6, 1, 1, 0.0, 0.0
				,GridBagConstraints.EAST, GridBagConstraints.NONE, new Insets(5, 5, 5, 0), 0, 0)); 
		kPanel.add(kBankAccountCombo, new GridBagConstraints(1, 6, 1, 1, 0.0, 0.0
				,GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(5, 5, 5, 5), 0, 0));
		//faaguilar OFB end
		...
		tBankAccountLabel.setText(Msg.translate(Env.getCtx(), "C_BankAccount_ID"));//faaguilar OFB
		tPanel.add(tBankAccountLabel, new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0
			,GridBagConstraints.EAST, GridBagConstraints.NONE, new Insets(5, 5, 5, 0), 0, 0));//faaguilar OFB change tAccountLabel
		tPanel.add(tBankAccountCombo, new GridBagConstraints(1, 0, 1, 1, 0.0, 0.0
			,GridBagConstraints.WEST, GridBagConstraints.NONE, new Insets(5, 5, 5, 5), 0, 0));//faaguilar OFB change tAccountCombo
		...
		//		,GridBagConstraints.CENTER, GridBagConstraints.NONE, new Insets(5, 5, 5, 5), 0, 0));//faaguilar Original comment
...
//				kAmountField.setReadWrite(!m_mPayment.isApproved());
				kTypeCombo.setReadWrite(true);
				kNumberField.setReadWrite(true);
				kNameField.setReadWrite(true);
				kExpField.setReadWrite(true);
				kApprovalField.setReadWrite(true);
				kOnline.setReadWrite(true);
				kAmountField.setReadWrite(true);
				//  Check
		 ...
		 *	Load Credit Cards
		 */
		//faaguilar OFB credit cards begin
				SQL = "SELECT value, name "
					+ "FROM AD_Ref_List "
					+ "WHERE isActive='Y' and AD_Reference_ID=149 ";
				...
				try
				{
					...
					while (rs.next())
					{
						...
						ValueNamePair pp = new ValueNamePair(rs.getString(1), rs.getString(2));
		...
		//faaguilar OFb credit cards end
		...
		//	Set Selection
		if (vp != null)
			kTypeCombo.setSelectedItem(vp);*/ //faaguilar Original Commented
		...
		if(MClient.get(Env.getCtx(), Env.getAD_Client_ID(Env.getCtx())).getName().equals("Muro") )
		{
			SQL = MRole.getDefault().addAccessSQL(
					"SELECT C_BankAccount_ID, Name || ' ' || AccountNo, IsDefault "
					+ "FROM C_BankAccount ba"
					+ " INNER JOIN C_Bank b ON (ba.C_Bank_ID=b.C_Bank_ID) "
					+ "WHERE b.IsActive='Y' and ba.C_BankAccount_ID=1000009 ",
					"ba", MRole.SQL_FULLYQUALIFIED, MRole.SQL_RO);
		}
		kp = null;
		try
		{
			...
			while (rs.next())
			{
				...
				kBankAccountCombo.addItem(pp);//faaguilar OFB
				tBankAccountCombo.addItem(pp);//faaguilar OFB
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		else if (e.getSource() == paymentCombo)
		{
			...
			if (pp != null)
			{
				...
				if(s.equals("g") || s.equals("e"))//faaguilar	
					s="t";//faaguilar
	...
	private boolean saveChangesInTrx(final String trxName){
		...
		String payTypes = m_Cash_As_Payment ? "KTSDBGE" : "KTSDGE"; //faaguilar add GE
		...
		else if (newPaymentRule.equals(X_C_Order.PAYMENTRULE_CreditCard))
		{
			...
			//faaguilar OFB begin bank for credit
			KeyNamePair kp = (KeyNamePair)kBankAccountCombo.getSelectedItem();
			if (kp != null)
				newC_BankAccount_ID = kp.getKey();
			//faaguilar OFB end bank for credit
		}
		...
		//	T (Transfer)	BPartner_Bank
			...
			|| newPaymentRule.equals("G")
			|| newPaymentRule.equals("E")) //faaguilar add G
		{
			tAccountCombo.getSelectedItem();
			KeyNamePair kp = (KeyNamePair)tBankAccountCombo.getSelectedItem();//faaguilar
			if (kp != null)//faaguilar
				newC_BankAccount_ID = kp.getKey();//faaguilar
		}
		...
		if (!newPaymentRule.equals(m_PaymentRule))
		{
			...
			//faaguilar desactivar linea si o si begin
			if (m_cashLine != null)
			{
			...
			else
			{
				...
				if (m_cashLine == null)
				{
					...
					else
					{
						...
						//ADialog.info(m_WindowNo, this, "m_cashLine - New Cashbook", "Amount: "+cl.getAmount());//faaguilar commented
						if (invoice != null)
						...
						newC_CashLine_ID = cl.getC_CashLine_ID(); //faaguilar
					}
				}
			}	//	have invoice
		}
		...
		//faaguilar OFB begin
		if ("KSDTGE".indexOf(newPaymentRule) != -1)  //faaguilar add GE
		{
			log.fine("Payment - " + newPaymentRule);
			//  Set Amount
			m_mPayment.setAmount(m_C_Currency_ID, payAmount);
			if (newPaymentRule.equals(MOrder.PAYMENTRULE_CreditCard))
			{
				...
				m_mPayment.setAmount(m_C_Currency_ID, (BigDecimal) kAmountField.getValue());
				m_mPayment.setPaymentProcessor();
				m_mPayment.setC_BankAccount_ID(newC_BankAccount_ID);//faaguilar OFB
			}
			else if (newPaymentRule.equals(MOrder.PAYMENTRULE_DirectDeposit)
				|| newPaymentRule.equals(MOrder.PAYMENTRULE_DirectDebit))
			{
//				m_mPayment.setBankACH(newC_BankAccount_ID, m_isSOTrx, newPaymentRule, 
//					tRoutingField.getText(), tNumberField.getText());  //faaguilar OFB commented
				m_mPayment.setTenderType(MPayment.TENDERTYPE_DirectDebit);//faaguilar OFB 
				m_mPayment.setAmount(m_C_Currency_ID, payAmount);
				m_mPayment.setC_BankAccount_ID(newC_BankAccount_ID);//faaguilar OFB
			}
			else if (newPaymentRule.equals("G") )//faaguilar OFB 
			{
				m_mPayment.setTenderType("G");
				m_mPayment.setAmount(m_C_Currency_ID, payAmount);
				m_mPayment.setC_BankAccount_ID(newC_BankAccount_ID);
			}//faaguilar OFB
			else if (newPaymentRule.equals("E") )//faaguilar OFB 
			{
				m_mPayment.setTenderType("B");
				m_mPayment.setAmount(m_C_Currency_ID, payAmount);
				m_mPayment.setC_BankAccount_ID(newC_BankAccount_ID);
			}//faaguilar OFB
			else if (newPaymentRule.equals(MOrder.PAYMENTRULE_Check))
		...
		if (newC_PaymentTerm_ID != m_C_PaymentTerm_ID)
		{
			m_mTab.setValue("C_PaymentTerm_ID", new Integer(newC_PaymentTerm_ID));
			 DB.executeUpdate("Update C_Invoice set C_PaymentTerm_ID="+ newC_PaymentTerm_ID  +"where C_Invoice_ID="+C_Invoice_ID, trxName);//faaguilar
		}
		//	Set Payment
		if (m_mPayment.getC_Payment_ID() != m_C_Payment_ID)
		{
			...
			else
			{
			    ...
			    DB.executeUpdate("Update C_Invoice set C_Payment_ID="+ m_mPayment.getC_Payment_ID()  +"where C_Invoice_ID="+C_Invoice_ID, trxName);//faaguilar
			}
		}
		//	Set Cash
	...
	private boolean checkMandatory()
	{
		...
		else if (PaymentRule.equals(MOrder.PAYMENTRULE_CreditCard))
		{
			...
			if (vp != null)
				CCType = vp.getValue();
	        String error="";
			if(kExpField.getText()==null || kExpField.getText().length()<2)//faaguilar
				error= "Fecha de Expiracion es Obligatoria";
			if (error.length() != 0)
			{
		...
		//	T (Transfer)	BPartner_Bank
			...
			|| PaymentRule.equals("G"))//faaguilar G
		{
...

			{
				...
				dataOK = false;
			}*///faaguilar original code commented
		...
		//	sCurrencyCombo.getSelectedItem();
			...
			//faaguilar OFB comment not necesary validation
			/*if (error.length() != 0)
			{
