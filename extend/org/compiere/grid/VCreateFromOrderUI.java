	...
	private void loadLines (int AD_User, int SubPro)
	{
		...
		if(DB.isPostgreSQL())
		{
			sql = "Select A.DocumentNo,A.DATEREQUIRED,A.AD_USER_ID,A.M_Requisition_ID,coalesce(C.M_PRODUCT_ID,0),(C.QTY-coalesce(C.QtyUsed,0)),C.M_Requisitionline_id,B.name,"
					+" coalesce(P.name,'') as Productname,C.PRICEACTUAL,C.C_SubProjectOFB_ID,pj.name as Projectname,C.description,coalesce(h.name,''),coalesce(h.C_Charge_ID,0) "
		...
		if(IsSOTrx)
		{
			//ininoles que no filtre por socio de negocio. 
