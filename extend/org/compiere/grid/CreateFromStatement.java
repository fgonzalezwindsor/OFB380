...
public class CreateFromStatement extends CreateFrom 
{
			...
			Object AmtFrom, Object AmtTo, Object DocType, Object TenderType, String AuthCode)
	{
		...
		//ininoles se agregan documentos asignados (A) para nuevo tipo de custodios CPV
		sql.append( " AND p.IsProtested='N' AND (p.Custodio is null OR p.Custodio IN('A') ) ");//faaguilar OFB  
	...
	throws SQLException
	{
		...
		log.config("** bankAccount: "+bankAccount.getC_BankAccount_ID());//faaguilar OFB
			...
			Object AmtFrom, Object AmtTo, Object DocType, Object TenderType, String AuthCode)
	{
		...
		String sql = "SELECT p.DateTrx,p.C_Payment_ID,p.DocumentNo, p.C_Currency_ID,c.ISO_Code, p.PayAmt,"
			...
			+ " bp.Name,l2.name as Tender " //faaguilar OFB TenderType
			...
			+ " LEFT OUTER JOIN AD_Ref_List l2 ON (p.TenderType=l2.value and l2.AD_Reference_ID=214) ";//faaguilar OFB
		...
		try
		{
			...
			while (rs.next())
			{
				...
				line.add(rs.getString(9));      	//faaguilar OFB 7-TenderType
				data.add(line);
			}
		}
	...
	protected void configureMiniTable (IMiniTable miniTable)
	{
		...
		miniTable.setColumnClass(7, String.class, true);    	//faaguilar OFB 7-TenderType
		//  Table UI
		miniTable.autoSize();
	}
	...
	protected Vector<String> getOISColumnNames()
	{
		...
		columnNames.add(Msg.translate(Env.getCtx(), "Tender"));//faaguilar OFB
