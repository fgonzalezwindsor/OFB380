...
public final class EMail implements Serializable
{
	...
	public String send ()
	{
		...
		try
		{
			...
			if (m_smtpHost.equalsIgnoreCase("smtpout.secureserver.net")) 
			{
				// TODO: make it configurable
				// Enable gmail port and ttls - Hardcoded
			...
			//ininoles se agrega host para prueba en DPP
			if (m_smtpHost.equalsIgnoreCase("10.16.25.238")) 
			{
				// TODO: make it configurable
				// Enable gmail port and ttls - Hardcoded
		...
		//	m_msg = new MimeMessage(session);
			...
			//ininoles agregar reply to a sistema con campo fax de usuario
			//ininoles se modifica para poder setear a posterior	
			if(m_replyTo == null)
			{
				MUser userRT = new MUser(m_ctx, Env.getAD_User_ID(m_ctx), null);
				String emailRT = null; 
				emailRT = userRT.getFax();
				if (emailRT != null && emailRT != "" && emailRT != " ")
				{	
					m_replyTo = new InternetAddress(emailRT, userRT.getName());				
				}
			}
			//			
			if (m_replyTo != null)
