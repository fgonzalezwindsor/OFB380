...
public class MAssetAcct extends X_A_Asset_Acct {
	...
	protected boolean afterSave(boolean newRecord, boolean success) {
		...
		/*try{
			DB.commit(false, get_TrxName());//faaguilar OFB fix
			} catch (Exception e) {	log.info("getAssets" + e);}*/
		...
		if (isProcessing() == true) {
			...
			//assetacct.setProcessing(true); faaguilar OFB comment original code
			assetacct.save();
	...
	/**faaguilar OFB
	 * metodo utilitario que retorna el registro contable de un activo especifico*/
	public static int getAssetAcct_ID(int A_Asset_ID){
		int revalue=0;
		revalue=DB.getSQLValue("A_Asset_Acct", "select a_asset_acct_id from a_asset_acct where a_asset_id="+A_Asset_ID);
