...
public class CalloutInOut extends CalloutEngine
{
	...
	public String product (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		else if (product.getM_Locator_ID() != 0)
		{
			...
			if (M_Warehouse_ID == loc.getM_Warehouse_ID() && loc.isActive() == true)//ininoles se agrega validacion para ubicaciones activas 
	...
	public String qty (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		else if (mField.getColumnName().equals("QtyEntered"))
		{
			...
			//faaguilar OFb qty validation begin
			int OrderLine_ID = Env.getContextAsInt(ctx, WindowNo, "C_OrderLine_ID");
			if(OrderLine_ID>0){
				int InOutLine_ID = Env.getContextAsInt(ctx, WindowNo, "M_InOutLine_ID");
				BigDecimal SoQty = DB.getSQLValueBD("C_Order","select QTYORDERED from C_OrderLine where C_OrderLine_ID=?",OrderLine_ID);//faaguilar OFb qty validation
				BigDecimal InOutQty = DB.getSQLValueBD("M_InOut","select sum(QtyEntered) from M_InOutLine where C_OrderLine_ID=? and M_InOutLine_ID!="+InOutLine_ID,OrderLine_ID);//faaguilar OFb qty validation
				 if(InOutQty ==null || InOutQty.signum()<0)
					 InOutQty=Env.ZERO;
				 if((InOutQty.add(QtyEntered)).compareTo(SoQty)>0)	
					    mTab.fireDataStatusEEvent ("Cantidad Mayor", "La Cantidad ingresada es mayor que la registrada en la Orden", false);
				
			}
			//faaguilar OFb qty validation end
