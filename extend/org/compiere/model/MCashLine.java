...
public class MCashLine extends X_C_CashLine
{
	...
	/**	Process Message 			*/
	private String		m_processMsg = null; //faaguilar OFB
	...
	{
		/**faaguilar OFB original code comented begin*/
		...
		/**faaguilar OFB original code comented end*/
		return true;
	}	//	beforeDelete
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		//faaguilar C_Payment Validation
		if (MCashLine.CASHTYPE_BankAccountTransfer.equals(getCashType()) && get_ValueAsString("DocumentNo")!=null)
		{
			if(get_ValueAsString("DocumentNo").length()>0){
				
				String mysql="select count(1) from c_payment where docstatus IN ('CO','DR') and isreceipt='"+(getAmount().signum()>0?"N":"Y")+"' and c_bpartner_id="+get_ValueAsInt("C_BPartner_ID")
	    		+ " and C_BankAccount_ID="+getC_BankAccount_ID()+" and checkno='"+get_ValueAsString("DocumentNo")+"'"+ (getC_Payment_ID()>0?" and C_payment_ID<>"+getC_Payment_ID():"");
				int count=DB.getSQLValue(get_TrxName(),mysql);
				if(count>0)
				{
					    log.saveError("Error", "Este Pago ya existe no puede ser generado desde una caja");
						m_processMsg = "Este Pago ya existe no puede ser generado desde una caja";
						return false;
					
				}
			}
		}
		if (MCashLine.CASHTYPE_BankAccountTransfer.equals(getCashType()) && getC_BankAccount_ID() == 0)
		{
			 log.saveError("Error", "la cuenta bancaria es obligatoria para este tipo de movimiento");
			  return false;
		}
		
		if (CASHTYPE_Invoice.equals(getCashType()) && getC_Invoice_ID() > 0 && newRecord)
		{
			
				log.config("Se hara asignacion de monto");
				log.config("id factura: "+getC_Invoice_ID());
				BigDecimal open=DB.getSQLValueBD(get_TrxName(), "select invoiceOpen(?, null) from C_Invoice WHERE C_Invoice_ID=?", getC_Invoice_ID(), getC_Invoice_ID());
				log.config("open despues de consulta: "+open+". factura: "+getC_Invoice_ID());
				if(open.signum()==0)
				{
					 log.saveError("Error", "esta Factura no posee monto abierto");
					  return false;
				}
				
				int count=DB.getSQLValue(get_TrxName(), "select count(1) from C_CashLine WHERE C_Cash_ID=? and C_Invoice_ID=? and C_CashLine_ID<>?", getC_Cash_ID(),getC_Invoice_ID(), getC_CashLine_ID());
				if(count>0)
				{
					 log.saveError("Error", "esta Factura ya se encuentra en otra linea");
					  return false;
				}
			
		}
		
		//validacion pago anulado
		if(is_ValueChanged(COLUMNNAME_C_Payment_ID) || newRecord ) //
		{
			if(getC_Payment_ID() > 0 )//se modifica validacion para que no de error ininoles
				if(!getC_Payment().getDocStatus().equals("CO") && !getC_Payment().getDocStatus().equals("CL"))
				{
					 log.saveError("Error", "No puede asignar un pago que esta anulado");
					  return false;
				}	
	...
	protected boolean afterSave (boolean newRecord, boolean success)
	{
		...
		if(isActive() && getC_Invoice_ID()>0)
			DB.executeUpdate("Update C_Invoice set C_CashLine_ID="+getC_CashLine_ID()  +"where C_Invoice_ID="+getC_Invoice_ID(), get_TrxName());//faaguilar
