...
public class MBPartnerLocation extends X_C_BPartner_Location
{
	...
	{
		//faaguilar OFB correct location name begin 
		MLocation address = getLocation(true);
		
		log.info("newRecord:"+newRecord);
		//	Set New Name
			if(address.getCity()!=null)
			setName (address.getAddress1()+";"+address.getCity());
			else if(address.getMCity()!=null)
			setName (address.getAddress1()+";"+address.getMCity().getName());
			else
			setName (address.getAddress1());
			
			log.info("mame:"+getName());
			return true;
		//faaguilar OFB correct location name end
	}
