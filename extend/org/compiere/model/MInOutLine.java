...
public class MInOutLine extends X_M_InOutLine
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		/**
		 * faaguilar OFB trae precio de compra para recibir y cotejar
		 * */
		//faaguilar OFB begin
		int group_IDAsset = 0;
		int group_IDAssetReceipt = get_ValueAsInt("A_Asset_Group_ID");
		boolean IsSetedAG = false;
		if(getC_OrderLine_ID()>0)
		{		
			MOrderLine oline= new MOrderLine(getCtx(),getC_OrderLine_ID(), get_TrxName());
			this.set_Value("PriceActual", oline.getPriceActual());
			...
			if (group_IDAsset > 0 && IsSetedAG == false)
			{
				set_CustomColumn("A_Asset_Group_ID", oline.get_ValueAsInt("A_Asset_Group_ID"));
				IsSetedAG = true;
			}
		...
		//faaguilar OFB asset validation
		if(getC_Charge_ID() != 0)
		{
					...
					if(charge.getC_ChargeType_ID()>0)
						if(charge.getC_ChargeType().getValue().equals("TCAF"))
						{
							...
							if(get_ValueAsString("A_CapvsExp").equals(X_C_InvoiceLine.A_CAPVSEXP_Expense))//gasto activo
							{
								if(get_ValueAsInt("A_Asset_ID")<=0)
								{
									log.saveError("Activo","Para gastos relacionados con un activos, el activo es obligatorio");
									return false;
								}
							}
							if(get_ValueAsString("A_CapvsExp").equals(X_C_InvoiceLine.A_CAPVSEXP_Capital) && IsSetedAG == false)//compra activo
							{								
	...
	public BigDecimal getBase (String CostDistribution)
	{
		if (MLandedCost.LANDEDCOSTDISTRIBUTION_Costs.equals(CostDistribution))
		{
			/**
			 * faaguilar OFB
			 * forma de distribucion de costo original
			 * comentada y reemplazada por la forma OFB*/
			...
			*/
			if(getParent().getC_Order_ID()==0){
				log.severe("Not Purchase Order");
				return Env.ZERO;
			}
				
			MOrder puo= new MOrder(getCtx(),getParent().getC_Order_ID(),get_TrxName());
			BigDecimal total=puo.getTotalLines();
			BigDecimal totalp=new BigDecimal("0.0");
			
			MOrderLine[] lines=puo.getLines("And M_Product_ID="+getM_Product_ID(), "");
	...
	{
		if(MInOutLineMA.get(getCtx(), getM_InOutLine_ID(), get_TrxName()).length>0)
			return true;
		
		if(getM_AttributeSetInstance_ID()!=0 && (getM_InOut().getMovementType().equals("C-") || getM_InOut().getMovementType().equals("V-")) )
		{
			
			MInOutLineMA ma = new MInOutLineMA (this,
