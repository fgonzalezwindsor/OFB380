...
public class MBankStatement extends X_C_BankStatement implements DocAction
{
	...
	{
		//begin faaguilar OFB duplicate
	 	int existe=0;
	 	String sql="select * from C_BankStatement where docstatus IN ('DR') "
						+ " and C_BANKACCOUNT_ID="+getC_BankAccount_ID() +" and C_BankStatement_ID!="+getC_BankStatement_ID();
		...
		if(existe==1 && !getDescription().equalsIgnoreCase("Reverso Protesto"))
		{
			log.saveError("Error", "Ya existe Un deposito en Borrador para esta cuenta y esta fecha, por favor completela antes de crear una nueva");
							m_processMsg = "Ya existe Un deposito en Borrador para esta cuenta y esta fecha, por favor completela antes de crear una nueva";
							return false;
		}
	    //END faaguilar OFB duplicate
		...
		/**ininoles solo actualiza si no esta completo o anulado, ya que se puede cambiar la descripcion una vez
		completado*/
		if (getDocStatus().compareToIgnoreCase("CO") != 0 && getDocStatus().compareToIgnoreCase("VO") != 0 )
		{
			setBeginningBalance(DB.getSQLValueBD(null,"select CURRENTBALANCE from C_BankAccount where C_BankAccount_ID=?",getC_BankAccount_ID()));//faaguilar OFB
