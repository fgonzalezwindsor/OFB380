	...
	implements DocAction, ProcessCall
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		//faaguilar OFB validation of exits..same information begin
		if(!is_ValueChanged("IsAllocated") && !getDocStatus().equals(STATUS_Completed)){
			int existe=0;
			...
			if(getTenderType().equals("K"))
			{
				if(isReceipt())
				{
						mysql="select count(1) from c_payment where docstatus IN ('CO','DR') and isreceipt='Y' and c_bpartner_id="+getC_BPartner_ID();
						if(getCheckNo()==null)
							mysql=mysql + " and CheckNo is null";
						else
							mysql=mysql + " and CheckNo='"+getCheckNo() +"'";
						if(getAccountNo()==null)
							mysql=mysql + " and	AccountNo is null ";
						else
							mysql=mysql + " and	AccountNo='"+ getAccountNo()+"'";
						
						if(getA_Name()!=null && getA_Name().length()>0)
							mysql=mysql + " and A_name='"+getA_Name()+"'";
						
							
						mysql=mysql +" and C_Payment_ID<>"+getC_Payment_ID();
									
						existe=DB.getSQLValue(null,mysql);
						if(existe>0)
						{
							log.saveError("Error", "Ya existe un cheque para este cliente con el mismo NoCheque y NoCuenta");
							m_processMsg = "Ya existe un cheque para este Cliente con el mismo NoCheque y NoCuenta";
							return false;
						}
			    ...
			    {
			    		mysql="select count(1) from c_payment where docstatus IN ('CO','DR') and isreceipt='N' and c_bpartner_id="+getC_BPartner_ID()
			    		+ " and C_BankAccount_ID="+getC_BankAccount_ID()+" and C_Payment_ID<>"+getC_Payment_ID();
						...
						{
							log.saveError("Error", "Ya existe un cheque para este cliente con el mismo NoCheque y NoCuenta");
							m_processMsg = "Ya existe un cheque para este Proveedor con el mismo NoCheque y NoCuenta";
							return false;
						}
			    }
				
			}
			else if(getTenderType().equals("C")) //credit crad
			{
				log.config("CreditCard no Validation");
			}
			else//Doc Number Validation
			{
				mysql="select count(1) from c_payment where docstatus IN ('CO','DR') "+
				" and DocumentNo='"+getDocumentNo() +"' and AD_Org_ID="+getAD_Org_ID() +" and C_Payment_ID<>"+getC_Payment_ID();
				
				if(isReceipt())
					mysql=mysql + " and isreceipt='Y'";
				else
					mysql=mysql + " and isreceipt='N'";
				...
				existe=DB.getSQLValue(null,mysql);
						...
						if(existe>0)
						{
							log.saveError("Error", "Ya existe un pago con este mismo numero de documento");
							m_processMsg = "Ya esta Registrado este numero de documento, favor revisar";
							return false;
						}
		...
		if(this.getPayAmt().signum()<0){
			log.saveError("Error", "El valor no puede ser Negativo");
			return false;
		}
		//end faaguilar OFB validation end
			...
			&& getC_Charge_ID() == 0)	//	allow different org for charge
		{
			...
			/*if (ba.getAD_Org_ID() != 0)   //begin
				setAD_Org_ID(ba.getAD_Org_ID());*/ //end
		}
		...
		if(this.getPayAmt().signum()!=0)
		{
			int decimals=this.getC_Currency().getStdPrecision();
			 setPayAmt(getPayAmt().setScale(decimals, BigDecimal.ROUND_HALF_EVEN));
...
*/
	...
	public String completeIt()
	{
		...
		//faaguilar OFB - Credit letter Import
		if(getTenderType().equals("I"))
		setPayAmt( (BigDecimal)this.get_Value("Amount"));
		// END faaguilar OFB - Credit letter Import
...
*/
		...
		//
		//ininoles crear diarios de CG para proyecto S31 GORE 
		/* --ya no se ocupan
		
		String sqltp = "select COUNT(*) from C_Payment cp "+
					"inner join c_paymentrequest pr on (cp.c_payment_id = pr.c_payment_id) "+
					"inner join c_project pj on (pr.c_project_id = pj.c_project_id) "+
					"inner join c_projecttype pt on (pj.c_projecttype_id = pt.c_projecttype_id) "+
					"where cp.C_Payment_ID = ? and help like 'S31'";
		
		int cant = DB.getSQLValue(get_TrxName(), sqltp, get_ID());
		
		if (cant > 0)
		{
			//Asiento 1 retenciones
			
			MJournal j1 = new MJournal(Env.getCtx(), 0, get_TrxName());
			j1.setGL_JournalBatch_ID(1000010);
			j1.setPostingType("A");
			j1.setDescription("CV D.TERC RETENC. " + get_ID());
			j1.setC_DocType_ID(1000000);
			//j.setDocStatus("DR");
			//j.setDocAction("CO");
			j1.setC_Currency_ID(228);
			j1.setC_AcctSchema_ID(1000000);
			j1.setGL_Category_ID(1000000);
			j1.setDateDoc(new Timestamp(TimeUtil.getToday().getTimeInMillis()));
			j1.setDateAcct(new Timestamp(TimeUtil.getToday().getTimeInMillis()));
			j1.setC_Period_ID(MPeriod.getC_Period_ID(Env.getCtx(), new Timestamp(TimeUtil.getToday().getTimeInMillis())));	
			j1.setC_ConversionType_ID(114);
			j1.set_CustomColumn("Type", "P");
			j1.set_CustomColumn("C_Payment_ID", get_ID());
			j1.set_CustomColumn("IsCustomAcct", "N");
			j1.save();			
			j1.set_CustomColumn("GL_JournalRef_ID", j1.get_ID()); //campo para poder acercarce a la contabildad de los journal
			j1.save();
			
			String sqlamt = "select abs(Coalesce(sum(pp.referenceamount), 0)) + abs(Coalesce(sum(pp.amt), 0)) " +
						"from c_payment cp inner join C_PaymentRequest pr on (cp.c_payment_id = pr.c_payment_id) "+
						...
						"where pr.c_payment_id = ?  and pay_type like 'R1'";
			
			BigDecimal amt = DB.getSQLValueBD(get_TrxName(), sqlamt, get_ID()) ;
			
			String sqlD1 = "SELECT C_Intermediation_Acct FROM C_AcctSchema_Default WHERE C_AcctSchema_ID=?";
			int IDAcountD1 = DB.getSQLValue(get_TrxName(), sqlD1, 1000000);
			
			String sqlC1 = "SELECT C_Guarantees_Acct FROM C_AcctSchema_Default WHERE C_AcctSchema_ID=?";
			int IDAcountC1 = DB.getSQLValue(get_TrxName(), sqlC1, 1000000);
			
			//Linea Debito 
			MJournalLine jl11 = new MJournalLine(j1);
			//jl.setGL_Journal_ID(j.get_ID());
			jl11.setC_ValidCombination_ID(IDAcountD1);
			jl11.setAmtSourceDr(amt);
			jl11.setAmtSourceCr(new BigDecimal("0.0"));
			jl11.setAmtAcct(amt,new BigDecimal("0.0"));
			jl11.setLine(10);			
			jl11.save();
			
			//Linea Credito 
			MJournalLine jl12 = new MJournalLine(j1);
			//jl.setGL_Journal_ID(j.get_ID());
			jl12.setC_ValidCombination_ID(IDAcountC1);
			jl12.setAmtSourceDr(new BigDecimal("0.0"));
			jl12.setAmtSourceCr(amt);
			jl12.setAmtAcct(new BigDecimal("0.0"),amt);
			jl12.setLine(20);			
			jl12.save();
			
			//end asiento 1
			
			//Asiento 2 multas cuenta puente
			
			MJournal j2 = new MJournal(Env.getCtx(), 0, get_TrxName());
			j2.setGL_JournalBatch_ID(1000010);
			j2.setPostingType("A");
			j2.setDescription("CV DEV " + get_ID());
			j2.setC_DocType_ID(1000000);
			//j.setDocStatus("DR");
			//j.setDocAction("CO");
			j2.setC_Currency_ID(228);
			j2.setC_AcctSchema_ID(1000000);
			j2.setGL_Category_ID(1000000);
			j2.setDateDoc(new Timestamp(TimeUtil.getToday().getTimeInMillis()));
			j2.setDateAcct(new Timestamp(TimeUtil.getToday().getTimeInMillis()));
			j2.setC_Period_ID(MPeriod.getC_Period_ID(Env.getCtx(), new Timestamp(TimeUtil.getToday().getTimeInMillis())));	
			j2.setC_ConversionType_ID(114);
			j2.set_CustomColumn("Type", "P");
			j2.set_CustomColumn("C_Payment_ID", get_ID());			
			j2.set_CustomColumn("IsCustomAcct", "N");
			j2.save();
			j2.set_CustomColumn("GL_JournalRef_ID", j2.get_ID());
			j2.save();
			
			String sqlamtMul = "select abs(Coalesce(sum(pp.amt), 0)) + abs(Coalesce(sum(pp.referenceamount), 0)) "+
						"from c_payment cp inner join C_PaymentRequest pr on (cp.c_payment_id = pr.c_payment_id) "+
						"inner join C_ProjectSchedule cps on (pr.C_ProjectSchedule_id = cps.C_ProjectSchedule_id) "+
						"left join PM_ProjectPay pp on (cps.C_ProjectSchedule_id = pp.C_ProjectSchedule_id) "+
						"where pr.c_payment_id = ? and pay_type like 'D%'";
			
			BigDecimal amtMul = DB.getSQLValueBD(get_TrxName(), sqlamtMul, get_ID()) ;
			
			String sqlD2 = "SELECT C_OtherRevenueC_Acct FROM C_AcctSchema_Default WHERE C_AcctSchema_ID=?";
			int IDAcountD2 = DB.getSQLValue(get_TrxName(), sqlD2, 1000000);
			
			String sqlC2 = "SELECT C_OtherRevenue_Acct FROM C_AcctSchema_Default WHERE C_AcctSchema_ID=?";
			int IDAcountC2 = DB.getSQLValue(get_TrxName(), sqlC2, 1000000);
			
			//Linea Debito 
			MJournalLine jl21 = new MJournalLine(j2);
			//jl.setGL_Journal_ID(j.get_ID());
			jl21.setC_ValidCombination_ID(IDAcountD2);
			jl21.setAmtSourceDr(amtMul);
			jl21.setAmtSourceCr(new BigDecimal("0.0"));
			jl21.setAmtAcct(amtMul,new BigDecimal("0.0"));
			jl21.setLine(10);			
			jl21.save();
			
			//Linea Credito 
			MJournalLine jl22 = new MJournalLine(j2);
			//jl.setGL_Journal_ID(j.get_ID());
			jl22.setC_ValidCombination_ID(IDAcountC2);
			jl22.setAmtSourceDr(new BigDecimal("0.0"));
			jl22.setAmtSourceCr(amtMul);
			jl22.setAmtAcct(new BigDecimal("0.0"),amtMul);
			jl22.setLine(20);			
			jl22.save();
			...
			MJournal j3 = new MJournal(Env.getCtx(), 0, get_TrxName());
			j3.setGL_JournalBatch_ID(1000010);
			j3.setPostingType("A");
			j3.setDescription("CV COMP. CTE " + get_ID());
			j3.setC_DocType_ID(1000000);
			//j.setDocStatus("DR");
			//j.setDocAction("CO");
			j3.setC_Currency_ID(228);
			j3.setC_AcctSchema_ID(1000000);
			j3.setGL_Category_ID(1000000);
			j3.setDateDoc(new Timestamp(TimeUtil.getToday().getTimeInMillis()));
			j3.setDateAcct(new Timestamp(TimeUtil.getToday().getTimeInMillis()));
			j3.setC_Period_ID(MPeriod.getC_Period_ID(Env.getCtx(), new Timestamp(TimeUtil.getToday().getTimeInMillis())));	
			j3.setC_ConversionType_ID(114);
			j3.set_CustomColumn("Type", "P");
			j3.set_CustomColumn("C_Payment_ID", get_ID());
			j3.set_CustomColumn("IsCustomAcct", "N");
			j3.save();
			j3.set_CustomColumn("GL_JournalRef_ID", j3.get_ID());
			j3.save();
									
			String sqlD3 = "select ch_expense_acct from c_payment cp "+
							"inner join c_charge cc on (cp.c_charge_id = cc.c_charge_id) "+
							"inner join C_Charge_Acct cca on (cca.c_charge_id = cc.c_charge_id) "+
							"where cp.c_payment_id = ? ";
			int IDAcountD3 = DB.getSQLValue(get_TrxName(), sqlD3, get_ID());
			
			int IDAcountC3 = IDAcountD2;
			
			//Linea Debito 
			MJournalLine jl31 = new MJournalLine(j3);
			//jl.setGL_Journal_ID(j.get_ID());
			jl31.setC_ValidCombination_ID(IDAcountD3);
			jl31.setAmtSourceDr(amtMul);
			jl31.setAmtSourceCr(new BigDecimal("0.0"));
			jl31.setAmtAcct(amtMul,new BigDecimal("0.0"));
			jl31.setLine(10);			
			jl31.save();
			
			//Linea Credito 
			MJournalLine jl32 = new MJournalLine(j3);
			//jl.setGL_Journal_ID(j.get_ID());
			jl32.setC_ValidCombination_ID(IDAcountC3);
			jl32.setAmtSourceDr(new BigDecimal("0.0"));
			jl32.setAmtSourceCr(amtMul);
			jl32.setAmtAcct(new BigDecimal("0.0"),amtMul);
			jl32.setLine(20);			
			jl32.save();			
			//end asiento 3			
		}
	...
	private boolean allocateInvoice()
	{
		...
		alloc.setAD_Org_ID(getAD_Org_ID());
		alloc.setDateAcct(getDateAcct());//faaguilar OFB
		alloc.saveEx();
		MAllocationLine aLine = null;
		if (isReceipt())
	...
	//		return;
		...
		for (int i = 0; i < allocations.length; i++)
		{
			...
			allocations[i].setDocAction(DocAction.ACTION_Void);//faaguilar OFB change DocAction.ACTION_Reverse_Correct by DocAction.ACTION_Void 
			if (!allocations[i].processIt(DocAction.ACTION_Void))//faaguilar OFB change DocAction.ACTION_Reverse_Correct by DocAction.ACTION_Void 
	...
	public boolean voidIt()
	{
		...
		/*if (getC_BankStatementLine_ID() > 0)
			return reverseCorrectIt();*/ //end original code
		
		//faaguilar OFB validate statement
		if (getC_BankStatementLine2_ID()>0)
	    {
	    	m_processMsg ="El pago se encuentra en un deposito, no puede ser anulado, revise sus depositos";
	    	return false;
	    }
	    //END faaguilar OFB validate statement	
		...
		{
			//Begin faaguilar OFB antes de anular begin
			if(!voidCashBook())
			{
				return false;
			}
			...
			if(reverseCorrectIt())
			{
				//faaguilar OFB resetpost
				int i=resetPost(getC_Payment_ID(),X_C_Payment.Table_ID);
	...
	public boolean reverseCorrectIt()
	{
		...
		//ininoles se manda error si periodo esta cerrado
		...
		/**faaguilar OFB codigo comentado creacion de pago inverso begin*/
		//	Create Reversal
		...
		*/
		/**faaguilar OFB codigo comentado creacion de pago inverso end*/
		...
		//	Set Status 
		//addDescription("(" + reversal.getDocumentNo() + "<-)"); //faaguilar OFB original code commented
		addDescription("Amount: " + getPayAmt());
		setDocStatus(DOCSTATUS_Voided);
		setDocAction(DOCACTION_None);
		setPayAmt(Env.ZERO);
		setProcessed(true);
		//FR [ 1948157  ] 
		//setReversal_ID(reversal.getC_Payment_ID()); //faaguilar OFB original code commented
		
		/**faaguilar OFB codigo comentado creacion de allocation pago inverso begin*/
		...
		*/
		/**faaguilar OFB codigo comentado creacion de allocation pago inverso end*/
		//	Update BPartner
	...
	{
		String sql = "select C_BankStatementLine_ID from c_bankstatementline A "
					+" inner join c_bankstatement B on (A.c_bankstatement_ID=B.c_bankstatement_ID) "
					+" where A.c_payment_id=?  and B.docstatus IN ('CO','DR','CL','IN')";
		int id = DB.getSQLValue(get_TrxName(), sql, getC_Payment_ID());
		if (id < 0)
			return 0;
		return id;
	}	//END faaguilar OFB custom method
	
	//faaguilar ofb custom method void cash book
	public boolean voidCashBook()
	{
		int cash_id =DB.getSQLValue(get_TrxName(),"select c_cash_id from c_cashline where c_payment_id="+getC_Payment_ID());
		if(cash_id>0)
		{
			...
			if(mycash.getDocStatus().equals("DR"))
			{
				...
				for (int i=0;i<lines.length;i++)
					if(lines[i].getC_Payment_ID()==getC_Payment_ID())
					{
			...
			else if(mycash.getDocStatus().equals("CO"))
			{
				...
				for (int i=0;i<lines.length;i++)
					if(lines[i].getC_Payment_ID()==getC_Payment_ID())
					{
						...
						newline.setDescription("Movimiento por Anulacion Pago:"+getDocumentNo());
						if(!isReceipt())
						newline.setCashType(MCashLine.CASHTYPE_GeneralExpense);
						else
						newline.setCashType(MCashLine.CASHTYPE_GeneralReceipts);	
	...
	public int getT_Conciliaciones_ID(){ //faaguilar OFB PDV
		int ReValue=0;
		ResultSet rs =null;
		PreparedStatement pstmt = null;
		int existe=DB.getSQLValue(get_TrxName(), "select count(1) from AD_Table where upper(tablename)=upper('T_CONCILIACIONES')");
		if(existe<=0)
			return 0;
		try
		{
	...
	public boolean isCompromisoIndirecto()
	{
		if(getC_Invoice_ID()>0){
			...
			MDocType doc= MDocType.get(getCtx(), invoice.getC_DocType_ID());
	...
	public void voidConciliacion(){ //faaguilar OFB PDV
		
	
		int existe=DB.getSQLValue(get_TrxName(), "select count(1) from AD_Table where upper(tablename)=upper('T_CONCILIACIONES')");
		if(existe<=0)
			return;
		int Conciliacion_ID=getT_Conciliaciones_ID();
		DB.executeUpdate("Update T_CONCILIACIONES set Docstatus='VO', DocAction='--' where T_CONCILIACIONES_ID="+Conciliacion_ID, get_TrxName());
		DB.executeUpdate("Update T_SOLICITUDES set ISRECONCILED='N'  where T_CONCILIACIONES_ID="+Conciliacion_ID, get_TrxName());
