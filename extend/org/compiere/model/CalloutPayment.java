...
public class CalloutPayment extends CalloutEngine
{
		...
		GridField mField, Object value)
	{
		...
		try
		{
			...
			if (rs.next ())
			{
				...
				mTab.setValue ("C_InvoicePaySchedule_ID", C_InvoicePaySchedule_ID);//faaguilar 
		...
		GridField mField, Object value, Object oldValue)
	{
			...
			&& mTab.get_ValueAsString ("DocStatus").equals ("DR"))
		{
			...
			//mTab.setValue ("WriteOffAmt", WriteOffAmt); comment by faaguilar OFB
			
			//faaguilar OFB SIEMPRE sub pagos begin
			mTab.setValue("IsOverUnderPayment","Y");
			mTab.setValue("OverUnderAmt",WriteOffAmt);
			//faaguilar OFB SIEMPRE sub pagos end
		}
	...
	/**
	 * faaguilar custom method SIN DESCUENTO DE FACTURA
	 * Payment_Invoice. when Invoice selected - set C_Currency_ID -
	 * C_BPartner_ID - DiscountAmt = C_Invoice_Discount (ID, DateTrx) - PayAmt =
	 * invoiceOpen (ID) - Discount - WriteOffAmt = 0
	 * @param ctx context
	 ...
	 * @return null or error message
	 */
	public String invoiceNotDiscount(Properties ctx, int WindowNo, GridTab mTab,
			GridField mField, Object value)
		{
			Integer C_Invoice_ID = (Integer)value;
			if (isCalloutActive () // assuming it is resetting value
			...
			try
			{
				...
				if (rs.next ())
				{
					...
					DiscountAmt = Env.ZERO; //faaguilar OFB siempre 0
					mTab.setValue ("PayAmt", InvoiceOpen.subtract (DiscountAmt));
					mTab.setValue ("DiscountAmt", DiscountAmt);
					// reset as dependent fields get reset
					Env.setContext (ctx, WindowNo, "C_Invoice_ID", C_Invoice_ID
