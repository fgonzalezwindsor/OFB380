...
public class MStorage extends X_M_Storage
{
		...
		boolean FiFo, String trxName)
	{
		...
		String sql = "SELECT * FROM M_Storage "
			...
			+ " AND QtyOnHand > 0 "		//faaguilar OFB <> by >0	
			+ "ORDER BY M_AttributeSetInstance_ID";
		if (!FiFo)
			sql += " DESC";
		
		sql += " ,QtyOnHand Desc"; //faaguilar OFB Transacciones duplicadas
		...
		boolean FiFo, boolean positiveOnly, int M_Locator_ID, String trxName)
	{
		...
		/**faaguilar OFB special order begin*/
		MClient client=MClient.get(ctx, Env.getAD_Client_ID(Env.getCtx()));
		/**faaguilar OFB special order end*/
		...
		if (allAttributeInstances)
		{
			...
			//faaguilar OFB special case to order begin REEMPLAZADO POR LA VENTANA DE PICKING 
			/*if(client.get_ValueAsBoolean("QtyPriority"))
			{
				sql += "AND (s.QtyOnHand - (select coalesce(sum(ol.qtyentered),0)  from m_inoutline ol inner join m_inout ou on (ol.m_inout_id=ou.m_inout_id) where ol.m_product_id="+M_Product_ID+" and ol.m_locator_id=l.M_Locator_ID and ou.docstatus='IP' and ou.movementtype='C-') )>0 "+
					" ORDER BY l.PriorityNo DESC,(select sum(ss.QtyOnHand) from M_Storage ss "+
 						"where ss.M_Product_ID=s.M_Product_ID and ss.M_Locator_ID=l.M_Locator_ID) ASC ";
			}*/
			// faaguilar OFB special case to order end
	 ...
	 * @return qty available (QtyOnHand-QtyReserved) or null if error
	 */
	public static BigDecimal getQtyAvailableProposal (int M_Warehouse_ID, int M_Locator_ID, 
		int M_Product_ID, int M_AttributeSetInstance_ID, String trxName)
	{
		BigDecimal retValue=Env.ZERO;
		
		ArrayList<Object> params = new ArrayList<Object>();
		StringBuffer sql = new StringBuffer("SELECT COALESCE(SUM(s.QtyOrdered),0)") // +s.QtyOnhand-s.QtyReserved
								.append(" FROM M_Storage s")
								.append(" WHERE s.M_Product_ID=?");
		params.add(M_Product_ID);
		// Warehouse level
		...
		//
		BigDecimal FutureQty = DB.getSQLValueBD(trxName, sql.toString(), params);
		//-------------------------
		params = new ArrayList<Object>();
		sql = new StringBuffer("SELECT COALESCE(SUM(s.QtyOrdered),0)")
		.append(" FROM C_OrderLine s")
		.append(" Inner join C_Order o on (s.C_Order_ID=o.C_Order_ID)")
		.append(" Inner join C_DocType doc on (o.C_DocType_ID=doc.C_DocType_ID)")
		.append(" WHERE o.DocStatus='CO' and doc.DocSubTypeSO='ON' and s.M_Product_ID=?");
		params.add(M_Product_ID);
		// Warehouse level
		if (M_Warehouse_ID > 0) {
		...
		BigDecimal ReserveQty = DB.getSQLValueBD(trxName, sql.toString(), params);
		if(ReserveQty==null)
			ReserveQty=Env.ZERO;
		...
		if (CLogMgt.isLevelFine())
			s_log.fine("M_Warehouse_ID=" + M_Warehouse_ID + ", M_Locator_ID=" + M_Locator_ID 
				+ ",M_Product_ID=" + M_Product_ID + " = " + retValue);
		return FutureQty.subtract(ReserveQty);
	}	//	getQtyAvailable
