	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		else
		{
			...
			while (rs.next()){
				...
				//ininoles que no se actualice balance si ya existe el registro
				String cantSql = "Select count(1) from A_Asset_Addition where A_Asset_Addition_ID = ? ";
				int cant = DB.getSQLValue(get_TrxName(), cantSql, getA_Asset_Addition_ID());
				if (cant > 0)
				{
					//no actualizar
				}
				else //solo actualizamos si no hay registro previo ininoles 
				{	
