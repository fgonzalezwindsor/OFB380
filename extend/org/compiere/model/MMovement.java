...
public class MMovement extends X_M_Movement implements DocAction
{
	...
	public String prepareIt()
	{
		...
		//faaguilar OFB begin
		 lines = getLinesProduct();
		 int lastProduct_ID=0;
		 int lastLoc_ID =0;
		 int lastAtt = 0;
		 BigDecimal acumQty = new BigDecimal("0.0");
				for(MMovementLine ll : lines){
					int abastLoc_ID = getLocatorTo(ll.getM_LocatorTo().getM_Warehouse_ID());
					
					if(ll.getM_LocatorTo_ID() == abastLoc_ID)
					{
						m_processMsg = "No Valid Locator line " + ll.getLine();
						return DocAction.STATUS_Invalid;
					}
					
					if(ll.getM_LocatorTo_ID()<=0)
					{
						m_processMsg = "No LocatorTO";
						return DocAction.STATUS_Invalid;
					}
					if(ll.getM_Locator_ID()==ll.getM_LocatorTo_ID())
					{
						m_processMsg = "Same Locator line " + ll.getLine();
						return DocAction.STATUS_Invalid;
					}
					...
					//validacion servicio
					MProduct prodV = new MProduct(getCtx(), ll.getM_Product_ID(),get_TrxName());
										
					if(acumQty.compareTo(getStock(ll.getM_Product_ID(), ll.getM_Locator_ID(), ll.getM_AttributeSetInstance_ID()))>0)
					{
						if(prodV.getProductType().equalsIgnoreCase("S"))
						{}
						else
						{
							m_processMsg = "Stock Insuficiente linea " + ll.getLine();
							return DocAction.STATUS_Invalid;
						}
					}
	...
	public boolean voidIt()
	{
		...
		if(getShipments()>0)//faaguilar OFB
		{
			m_processMsg = "Documento Con Entregas Relacionadas Procesadas";
			return false;
		}
		delShipment();//faaguilar OFB
	...
	/**faaguilar OFB copyline from movement*/
	...
	private void processShipment(){
		...
		{
			DB.executeUpdate("update m_inoutline " +
					"set m_locator_id="+line.getM_LocatorTo_ID()
					+ " where M_MovementLine_ID="+line.getM_MovementLine_ID(), get_TrxName());
		}
		//***
		
		String sql = "select * from m_inout "
			+" where docstatus='DR' and M_Movement_ID=?";
		...
		try
		{
			...
			while(rs.next()){
				MInOut out = new MInOut(getCtx(),rs,get_TrxName());
				out.processIt("PR");
				out.save();
			}
   ...
   private void delShipment(){
		...
		try
		{
			...
			while(rs.next()){
				MInOut out = new MInOut(getCtx(),rs,get_TrxName());
				//out.delete(true, get_TrxName());
				out.deleteEx(true, get_TrxName());
			}
   ...
   private int getShipments(){
		...
		String sql = "select count(1) from m_inout "
			+" where docstatus IN ('CO','IP') and M_Movement_ID=?";
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try
		{
    ...
    private BigDecimal getStock(int M_Product_ID, int M_Locator_ID, int M_AttributeSetInstance_ID)
    {
		...
		if(M_AttributeSetInstance_ID>0)
			mySQL+=" and M_AttributeSetInstance_ID="+M_AttributeSetInstance_ID;
		
		BigDecimal stock = DB.getSQLValueBD(get_TrxName(), mySQL, M_Locator_ID,M_Product_ID);
		if(stock==null)
			stock=Env.ZERO;
