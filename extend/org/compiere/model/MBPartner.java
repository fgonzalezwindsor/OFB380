...
public class MBPartner extends X_C_BPartner
{
	...
	public MBPartner (X_I_BPartnerXML impBP)
	{
		this (impBP.getCtx(), 0, impBP.get_TrxName());
		setClientOrg(impBP);
		setUpdatedBy(impBP.getUpdatedBy());
	...
	public void setTotalOpenBalance ()
	{
		...
		//faaguilar OFB validacion de optimizacion de BP
		if(getRating ()!=null)
			if(getRating ().equals("X"))
			return;
		//fin validacion
		/*faaguilar OFB SQL original comentado
		...
		String sql = "SELECT "
			...
			+ "WHERE C_BPartner_ID=?";		
		*/
		//faaguilar OFB new SQL
		String sql ="SELECT COALESCE((SELECT SUM(currencyBase(invoiceOpen(i.C_Invoice_ID,i.C_InvoicePaySchedule_ID),i.C_Currency_ID,i.DateInvoiced, i.AD_Client_ID,i.AD_Org_ID)) FROM C_Invoice_v i "+
			" WHERE i.C_BPartner_ID=bp.C_BPartner_ID AND i.IsSOTrx='Y' AND i.C_Order_ID is null AND i.Extinta='N'),0) + "+
			" COALESCE((SELECT SUM(currencyBase(invoiceOpen(i.C_Invoice_ID,i.C_InvoicePaySchedule_ID),i.C_Currency_ID,i.DateInvoiced, i.AD_Client_ID,i.AD_Org_ID)) FROM C_Invoice_v i "+
			" WHERE i.C_BPartner_ID=bp.C_BPartner_ID AND i.IsSOTrx='Y' AND i.Extinta='N' AND i.C_Order_ID is not null AND not exists (select * from C_Orderline sol where qtyordered<>qtyinvoiced and  "+
			" i.c_order_id=sol.c_order_id)),0) +"+
			" COALESCE((SELECT SUM(currencyBase(o.GrandTotal,o.c_currency_id,o.dateordered,o.ad_client_id,o.ad_org_id)-orderpay(o.c_order_id)) "+
			" FROM C_Order o inner join C_Invoice inv on (o.C_order_ID=inv.C_Order_ID and  inv.DocStatus='CO' AND inv.Extinta='N') "+
			" WHERE o.C_BPartner_ID=bp.C_BPartner_ID AND o.IsSOTrx='Y' AND o.DocStatus='CO' And exists (select * from C_Orderline sol where qtyordered>qtyinvoiced and "+
			" sol.c_order_id=o.c_order_id)),0) +"+
			" COALESCE((SELECT SUM(currencyBase(o.GrandTotal,o.c_currency_id,o.dateordered,o.ad_client_id,o.ad_org_id)) "+
			" FROM C_Order o "+
			" WHERE o.C_BPartner_ID=bp.C_BPartner_ID AND o.IsSOTrx='Y' AND o.DocStatus='CO' "+
			" And 0= (select sum(qtyinvoiced) from C_Orderline sol where sol.c_order_id=o.c_order_id)),0)  as creditused, "+
			" (COALESCE((SELECT SUM(currencyBase(invoiceOpen(i.C_Invoice_ID,i.C_InvoicePaySchedule_ID),i.C_Currency_ID,i.DateInvoiced, i.AD_Client_ID,i.AD_Org_ID)) FROM C_Invoice_v i "+
			" WHERE i.C_BPartner_ID=bp.C_BPartner_ID AND i.IsSOTrx='Y' AND i.Extinta='N' AND  i.C_Order_ID is null),0) + "+
			" COALESCE((SELECT SUM(currencyBase(invoiceOpen(i.C_Invoice_ID,i.C_InvoicePaySchedule_ID),i.C_Currency_ID,i.DateInvoiced, i.AD_Client_ID,i.AD_Org_ID)) FROM C_Invoice_v i "+
			" WHERE i.C_BPartner_ID=bp.C_BPartner_ID AND i.IsSOTrx='Y' AND i.Extinta='N' AND i.C_Order_ID is not null AND not exists (select * from C_Orderline sol where qtyordered<>qtyinvoiced and  "+
			" i.c_order_id=sol.c_order_id)),0))-"+
			" COALESCE((SELECT SUM(currencyBase(Paymentavailable(p.C_Payment_ID),p.C_Currency_ID,p.DateTrx,p.AD_Client_ID,p.AD_Org_ID)) FROM C_Payment_v p "+
			" WHERE p.C_BPartner_ID=bp.C_BPartner_ID AND p.IsAllocated='N' "+
			" AND p.C_Charge_ID IS NULL),0)  as totalbalance"+
			" FROM C_BPartner bp "+
			" WHERE C_BPartner_ID=?";
         //faaguilar OFB new SQL END
		PreparedStatement pstmt = null;
		try
		{
			pstmt = DB.prepareStatement (sql, get_TrxName());
