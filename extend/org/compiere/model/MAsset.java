	...
	protected boolean afterSave (boolean newRecord,boolean success)
	{
		...
		{		    	
			...
			try {
				...
				MAssetAcct assetacct = null;
				while (rs.next())
				{
					MAssetGroupAcct assetgrpacct = new MAssetGroupAcct (getCtx(), rs, get_TrxName());
					assetacct = new MAssetAcct (getCtx(), 0, get_TrxName());			
					isdepreciate = assetgrpacct.isActive(); //faaguilar change active by processed
					if (isDepreciated()== true | isdepreciate == true)
					{		
						//validacion para que no sobrescriba assetacct ininoles
						int cantAcct = DB.getSQLValue(get_TrxName(), "SELECT COUNT(1) FROM A_Asset_Acct WHERE A_Asset_ID = "+get_ID()+" AND IsActive = 'Y'");
						if (cantAcct < 1)
						{
							...
							log.config("variables al crear activo: "+getUseLifeMonths()+"-"
									+assetacct.get_ValueAsBoolean("IsByYear")+"-"+getUseLifeYears()+"-ID asseAcct"+assetacct.get_ID());
	
							if (getUseLifeMonths() == 0 & getUseLifeYears() == 0 & assetgrpacct.get_ValueAsBoolean("IsByYear"))
							{
								log.config("dentro de if (getUseLifeMonths() == 0 & getUseLifeYears() == 0)");
								assetacct.setA_Period_End(assetgrpacct.getUseLifeYears());
							...
							else if (getUseLifeMonths() == 0 & getUseLifeYears() == 0)
							{
								log.config("dentro de if (getUseLifeMonths() == 0 & getUseLifeYears() == 0)");
							...
							else if(getUseLifeYears() > 0 & assetacct.get_ValueAsBoolean("IsByYear"))
							{
								log.config("dentro de 2° if");
								assetacct.setA_Period_End(getUseLifeYears());
							...
							//faaguilar OFB new accounting Begin
							/*assetacct.set_Value("Salvage_Value",assetgrpacct.get_ValueAsInt("Salvage_Value")); 
							assetacct.set_Value("Disposal_RevenueD_Acct",assetgrpacct.get_ValueAsInt("Disposal_RevenueD_Acct"));
							assetacct.set_Value("Disposal_RevenueC_Acct",assetgrpacct.get_ValueAsInt("Disposal_RevenueC_Acct"));
							assetacct.set_Value("Disposal_Loss_Acct",assetgrpacct.get_ValueAsInt("Disposal_Loss_Acct"));
							assetacct.set_Value("Disposal_Gain_Acct",assetgrpacct.get_ValueAsInt("Disposal_Gain_Acct"));
							assetacct.set_Value("AssetComplement_Acct",assetgrpacct.get_ValueAsInt("AssetComplement_Acct"));*/
							
							assetacct.set_Value("A_Salvage_Value",(BigDecimal)assetgrpacct.get_Value("A_Salvage_Value"));//ininoles 
							...
							assetacct.set_Value("IsByYear", assetgrpacct.get_Value("IsByYear"));
							//faaguilar OFB new accounting End
				...
				change.setA_QTY_Current(Env.ONE);//faaguilar OFB
			    change.setA_QTY_Original(Env.ONE);//faaguilar OFB
				change.save();
				
				//faaguilar OFB Plan Begin
				CreateAssetForecast.createForecast(asset, change, assetacct, get_TrxName());
				//faaguilar OFB Plan End
		...
		else 
		{
			//int uselifemonths = 0;
			//int uselifeyears = 0;		
			sql ="SELECT A_Asset_Acct_ID FROM A_Asset_Acct WHERE A_Asset_ID = ? AND IsActive='Y'";		
			pstmt = null;
			pstmt = DB.prepareStatement(sql,get_TrxName());
			...
			try {
				...
				ResultSet rs = pstmt.executeQuery();	
				while (rs.next())
				{			//ininoles puebas descomentar posteriormente
					/*MAssetAcct assetacct = new MAssetAcct (getCtx(), rs.getInt("A_Asset_Acct_ID"), get_TrxName());			
					assetacct.setProcessing(false); 
					//assetacct.setA_Asset_ID(p_A_Asset_ID);						
					assetacct.setA_Period_Start(1);			
					if (getUseLifeYears() > 0 & assetacct.get_ValueAsBoolean("IsByYear"))
					{
						assetacct.setA_Period_End(getUseLifeYears());
						setUseLifeMonths(0);
						uselifemonths = 0;
