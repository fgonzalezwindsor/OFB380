...
public class MCost extends X_M_Cost
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		log.config("beforeSave: Product_ID:" +getM_Product_ID() + " Org_ID: "+getAD_Org_ID());
		if (m_manual)
		{
		...
		if (MAcctSchema.COSTINGLEVEL_Organization .equals(CostingLevel))
		{
			...
			{
				log.saveError("CostingLevelCostCenter", "");
				return false;
			}
		}
		//faaguilar OFB end
	...
	/**
	 * faaguilar OFB  getcost*/
	public static BigDecimal getCost (MProduct product,
			int M_AttributeSetInstance_ID,
			MAcctSchema as, int AD_Org_ID, String costingMethod,
			BigDecimal qty, 
			boolean zeroCostsOK,Timestamp DateAct, String trxName)
	{
		String CostingLevel = product.getCostingLevel(as);
		if (MAcctSchema.COSTINGLEVEL_Client.equals(CostingLevel))
		{
