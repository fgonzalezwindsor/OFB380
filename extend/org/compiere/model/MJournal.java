...
public class MJournal extends X_GL_Journal implements DocAction
{
	...
	public int copyLinesFrom (MJournal fromJournal, Timestamp dateAcct, char typeCR)
	{
		...
		for (int i = 0; i < fromLines.length; i++)
		{
			...
			toLine.setAD_Org_ID(fromLines[i].getAD_Org_ID());//faaguilar OFB
			if (toLine.save())
				count++;
		}
	...
	public String prepareIt()
	{
		...
		try{
			UseUn = gl.get_ValueAsBoolean("UseUnbalancedAcct");
			
		}
		catch (Exception e)
		{
			log.severe("Could not asigment UseUnbalancedAcct - " + e.getMessage());
			UseUn = false;
		}
		...
		{			
			...
			{
				//ininoles se agrega nuevo ticket para que complete diario desbalanceados
				if(gl.isUseSuspenseBalancing() == false && UseUn == false)
				{
