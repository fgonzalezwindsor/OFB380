...
public class CalloutInvoice extends CalloutEngine
{
	...
	public String paymentTerm (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		//ininoles se cambian valores de cadena por bolean
	...
	public String pricelistReprice (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		if(retValue==0)
		{
			log.info("No pricelistversion Reprice");
			return "";
		}
		...
		if(CurrentCurrency_ID==228)
			return "";
		int CurTo_ID=((Integer)mTab.getValue("C_Currency_ID")).intValue();
		...
		MultiplyRate=MConversionRate.getRate(CurrentCurrency_ID,CurTo_ID,priceDate, ((Integer)mTab.getValue("C_ConversionType_ID")).intValue(), 
			Env.getContextAsInt(ctx, WindowNo, "AD_Client_ID"), Env.getContextAsInt(ctx, WindowNo, "AD_Org_ID"));	
		//CLP=228
		log.config("currency=100 - To_Currency="+CurTo_ID + "- Date="+priceDate+"-Conversion="+((Integer)mTab.getValue("C_ConversionType_ID")).intValue());
		...
		String sql2 = "SELECT invl.C_InvoiceLine_ID "
                +" FROM C_InvoiceLine invl "
                +" WHERE invl.C_Invoice_ID="+ Invoice_ID;	
        
        log.config("C_Invoice_ID:"+Invoice_ID+"-MultiplyRate:"+MultiplyRate);
        
        mTab.setValue("MultiplyRate", MultiplyRate);
        
        if( mTab.getValue("ForeignPrice")==null || ((BigDecimal)mTab.getValue("ForeignPrice")).signum()==0)
        	 mTab.setValue("ForeignPrice", mTab.getValue("GrandTotal"));//ForeignPrice
		...
		try
		{
			...
			while (rs2.next ())
			{
				...
				//fin validacion
				line.set_Value("ForeignPrice", line.getPriceEntered());
				...
				line.setPriceActual(line.getPriceActual().multiply(MultiplyRate).setScale(curreTo.getStdPrecision(),BigDecimal.ROUND_HALF_EVEN));
				line.setPriceEntered(line.getPriceEntered().multiply(MultiplyRate).setScale(curreTo.getStdPrecision(),BigDecimal.ROUND_HALF_EVEN));
