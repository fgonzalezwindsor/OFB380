...
public class CalloutMovement extends CalloutEngine
{
	...
	public String qty(Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value) {
		...
		checkQtyAvailableOFB(ctx, mTab, WindowNo, M_Product_ID, (BigDecimal)value);//faaguilar OFB
		//
		return "";
	} //  qty
	...
	public String locator(Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value) {
		...
		int M_Product_ID = Env.getContextAsInt(ctx, WindowNo, "M_Product_ID");
		//checkQtyAvailable(ctx, mTab, WindowNo, M_Product_ID, null);
		checkQtyAvailableOFB(ctx, mTab, WindowNo, M_Product_ID, null);//faaguilar OFB
		return "";
	}
	...
	private void checkQtyAvailableOFB(Properties ctx, GridTab mTab, int WindowNo, int M_Product_ID, BigDecimal MovementQty) {
		...
		if (M_Product_ID != 0) {
			...
			if (product.isStocked()) {
				...
				BigDecimal stock = DB.getSQLValueBD(null, mySQL, M_Locator_ID,M_Product_ID);
				if (stock.compareTo(MovementQty) < 0)
