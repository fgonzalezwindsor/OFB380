...
public class MCostDetail extends X_M_CostDetail
{
	...
	public synchronized boolean process()
	{
		...
		return true;//faaguilar OFB
		
		/*
		//faaguilar OFB Begin
		try{
		if(!((Boolean)this.get_Value("Enabled"))){
		...
		**/
		...
		else if (getC_InvoiceLine_ID() != 0)
		{
			...
			else if (ce.isStandardCosting())
			{
				...
				cost.add(amt, qty);
				//faaguilar OFB begin
				if(cost.getCurrentQty().signum()!=0)
					cost.setCurrentCostPrice(cost.getCumulatedAmt().divide(cost.getCurrentQty(), 2, BigDecimal.ROUND_HALF_EVEN));
				//faaguilar OFB end
		...
		//faaguilar OFB begin GL_JournalLine_ID
		else if (((Integer)get_Value("GL_JournalLine_ID")).intValue()>0){
		...
		//faaguilar OFB end GL_JournalLine_ID
	...
	/**
	 * faaguilar OFB
	 * resetea el costo relaciona con una linea de factura
	 * usado cuando la factura de compra es anulada*/
	public synchronized boolean voidline()
	{
		if (getQty().signum()==0 &&  getAmt().signum()==0)
		{
			log.info("Already reset");
			return true;
		}
		if (!isProcessed()){
			setQty(Env.ZERO);
		...
		if (getM_CostElement_ID() == 0)
		{
			...
			for (int i = 0; i < ces.length; i++)
			{
				...
				if (ce.isStandardCosting())
				{
					cost = MCost.get(product, M_ASI_ID, as, 
							Org_ID, ce.getM_CostElement_ID(),product.get_TrxName());
				}
			}
		}	//	Material Cost elements
		else
		...
		cost.add(getAmt().negate(), getQty().negate());
		cost.save();
		//	Save it
		if (ok)
		{
