...
public class MInvoiceLine extends X_C_InvoiceLine
{
	...
	public void setOrderLine (MOrderLine oLine)
	{
		...
		setAD_Org_ID(oLine.getAD_Org_ID());//OFB faaguilar copy line org
	}	//	setOrderLine
	...
	//	setLineNetAmt();
		...
		/*if (tax.isDocumentLevel() && m_IsSOTrx)		//	AR Inv Tax
			return;*///faaguilar OFB original code commented
		//
		if(!m_IsSOTrx)//faaguilar OFB
			TaxAmt = tax.calculateTax(getLineNetAmt(), isTaxIncluded(), getPrecision());
		else
			TaxAmt = tax.calculateTax(getLineNetAmt(), isTaxIncluded(), getPrecision());//faaguilar OFB
		
		if (isTaxIncluded())
			setLineTotalAmt(getLineNetAmt());
		else
			setLineTotalAmt(getLineNetAmt().add(TaxAmt).setScale(getPrecision(), BigDecimal.ROUND_HALF_UP));//faaguilar OFB scale
		super.setTaxAmt (TaxAmt);//faaguilar OFB scale
	}	//	setTaxAmt
	...
	public void setLineNetAmt ()
	{
		...
		if(getParent().getC_Currency_ID()==228 )
		bd = bd.setScale(0, BigDecimal.ROUND_HALF_UP);//faaguilar OFB
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		{
			if(!OFBForward.NoValidationPriceInvoiceLineZero())
			{
				if (!m_priceSet
		...
		if(getC_OrderLine_ID()>0 )
		{
			...
			set_CustomColumn("A_CapvsExp", oLine.get_Value("A_CapvsExp"));
			set_CustomColumn("A_Asset_Group_ID", oLine.get_Value("A_Asset_Group_ID"));
			group_IDAsset = oLine.get_ValueAsInt("A_Asset_Group_ID");
			set_CustomColumn("A_Asset_ID", oLine.get_Value("A_Asset_ID"));
			set_CustomColumn("A_CreateAsset", oLine.get_Value("A_CreateAsset"));
		}		
		if	(getM_InOutLine_ID()>0){
			MInOutLine iLine = new MInOutLine(getCtx(),getM_InOutLine_ID(),get_TrxName());
			if(iLine.get_ValueAsString("Help")!=null && !iLine.get_ValueAsString("Help").equals(""))
				this.set_CustomColumn("Help",iLine.get_ValueAsString("Help") );
			
			if(iLine.get_ValueAsInt("A_Asset_ID")>0)
				this.set_CustomColumn("A_Asset_ID",iLine.get_ValueAsInt("A_Asset_ID") );
			
			set_CustomColumn("A_CapvsExp", iLine.get_Value("A_CapvsExp"));
			set_CustomColumn("A_Asset_Group_ID", iLine.get_Value("A_Asset_Group_ID"));
			group_IDAsset = iLine.get_ValueAsInt("A_Asset_Group_ID");
			set_CustomColumn("A_Asset_ID", iLine.get_Value("A_Asset_ID"));
			set_CustomColumn("A_CreateAsset", iLine.get_Value("A_CreateAsset"));
		}
		
		if(getC_Charge_ID()>0)
			  if(getC_Charge().getC_ChargeType_ID()>0)
				if( getC_Charge().getC_ChargeType().getValue().equals("TCAF"))
				{
	...
	public String allocateLandedCosts()
	{
		...
		if (lcs.length == 1)
		{
			...
			if (lc.getM_InOut_ID() != 0 && lc.getM_InOutLine_ID() == 0)
			{
				...
				for (int i = 0; i < list.size(); i++)
				{
					...
					if (base.signum() != 0)
					{
						...
						lca.setAmt(result, 3);//faaguilar OFB
			...
			else if (lc.getM_Product_ID() != 0)
			{
				...
				lca.setAmt(getLineNetAmt());
				lca.setBase(getLineNetAmt());//faaguilar OFB
				lca.setQty(Env.ZERO);//faaguilar OFB
				if (lca.save())
					return "";
