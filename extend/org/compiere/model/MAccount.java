...
public class MAccount extends X_C_ValidCombination
{
	...
	public void setValueDescription()
	{
		...
		String custom = new String(); //faaguilar OFB
		...
		for (int i = 0; i < elements.length; i++)
		{
			...
			else if (MAcctSchemaElement.ELEMENTTYPE_Account.equals(element.getElementType()))
			{
				if (getAccount_ID() != 0)
				{
					...
					custom=m_accountEV.getValue() +"-"+m_accountEV.getName();//faaguilar OFB
				}
		 ...
		 */
		super.setCombination(custom);//faaguilar OFB
