...
public final class ImpFormat
{
	...
	public boolean updateDB (Properties ctx, String line, String trxName, int row)
	{
		...
		String[] nodes = parseLine (line, true, false, true, row);	//	with label, no trace, ignore empty
		if (nodes.length == 0)
		{
			log.finest("Nothing parsed from: " + line);
			return false;
	...
	public String[] parseLine (String line, boolean withLabel, boolean trace, boolean ignoreEmpty, int fila)
	{
		...
		if(this.m_name.toUpperCase().indexOf("SANTIAGO")>-1 && fila==0)//faaguilar OFB
		{
			this.emision = line.substring(162-1, 170);
			this.folio = line.substring(82-1, 85);
			this.saldo = line.substring(146-1, 160);
			this.ccorriente = line.substring(70-1, 81);
		}
		else if(this.m_name.toUpperCase().indexOf("CHILE")>-1 && fila==2)//faaguilar OFB
		{
			this.saldo = line.substring(37-1, 48);
		}
		else if(this.m_name.toUpperCase().indexOf("BCI")>-1 && fila==0)//faaguilar OFB
		{
			this.emision = line.substring(14-1, 22);
			this.folio = line.substring(22-1, 26);
			this.saldo = line.substring(75-1, 87);
		}
		...
		if(this.saldo!=null && this.saldo.length()>0)
			list.add("SaldoInicial = " + parseNumber(this.saldo)  );
		if(this.folio!=null && this.folio.length()>0)
			list.add("Folio = " + parseNumber(this.folio)  );
		if(this.emision!=null && this.emision.length()>0)
			list.add("documentdate = TO_DATE('" + parseDate(this.emision) + "','YYYY-MM-DD HH24:MI:SS')");
