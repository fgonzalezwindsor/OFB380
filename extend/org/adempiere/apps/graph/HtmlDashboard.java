...
import org.ofb.model.MDashboardTable;
...
public class HtmlDashboard extends JPanel implements MouseListener,
	...
	private enum PAGE_TYPE {PAGE_HOME, PAGE_PERFORMANCE, PAGE_LOGO, PAGE_OFB};//faaguilar page OFB
	...
	private String createHTML(PAGE_TYPE requestPage){
		...
		switch (requestPage) {
			...
			case PAGE_HOME: //**************************************************************
				...
				break;
			case PAGE_OFB: //faaguilar begin
				
				result += "</head><body><div class=\"content\">\n";
				String sql1 =  " SELECT * "
						+ " FROM OFB_DashboardTable x"
						+ " WHERE (x.AD_Client_ID=0 OR x.AD_Client_ID=?) AND x.IsActive='Y'" +
						" Order By Line";
				...
				try
				{
					...
					pstmt1.setInt(1, Env.getAD_Client_ID(Env.getCtx()));
					rs1 = pstmt1.executeQuery();
					while (rs1.next()) {				
					
						MDashboardTable table = new MDashboardTable(Env.getCtx(),rs1,null);
						X_OFB_DashboardTableCol[] cols = table.getTableCols();
						htmlTable.append("<table class=\"dataGrid\"><tr>")
							.append("<th colspan=\""+(cols.length+1)+"\" class=\"label\"><b>"+ table.getTableName() +"</b></th></tr>")
						 	.append("<tr>")
							.append("<th scope=\"col\"></th>");
						
						for(int i =0;i<cols.length;i++)//HEADERS
							htmlTable.append("<th scope=\"col\">"+ cols[i].getColName() +"</th>");
						htmlTable.append("</tr>");
						
						htmlTable.append("<tr>")
						.append("<td class=\"label\">"+ table.getNameRow1() +"</td>");
						for(int i =0;i<cols.length;i++)//VALUES
							htmlTable.append("<td  class=\"tdcontent\">"+ table.getColTotal(cols[i].getOFB_DashboardTableCol_ID(),0) +"</td>");
						htmlTable.append("</tr>");
						
						if(table.getMeasureDisplay().equals(X_OFB_DashboardTable.MEASUREDISPLAY_Month))//Extra VALUES
						{
							htmlTable.append("<tr>")
							.append("<td class=\"label\">"+ table.getNameRow2() +"</td>");
							for(int i =0;i<cols.length;i++)//VALUES
								htmlTable.append("<td  class=\"tdcontent\">"+ table.getColTotal(cols[i].getOFB_DashboardTableCol_ID(),1) +"</td>");
							htmlTable.append("</tr>");
						}
				...
				finally
				{
				 ...
				 //result ="<html><body><div class=\"content\">";
				
				 /*result += new StringBuffer("<table class=\"dataGrid\"><tr>")
					.append("<th colspan=\""+columns+"\" class=\"label\"><b>Resumenes</b></th></tr>")
					.append("<tr>")
					.append("<th scope=\"col\"></th>")
					.append("<th scope=\"col\">Ordenes de Compra</th>")
					.append("<th scope=\"col\">Facturas de Compra</th>")
					.append("<th scope=\"col\">Facturas de Venta</th>")
					.append("<th scope=\"col\">Pagos Recibidos</th>")
					.append("</tr>")
					.append("<tr>")
					.append("<td class=\"label\">Actual</td>")
					.append("<td  class=\"tdcontent\">0</td>")
					.append("<td  class=\"tdcontent\">0</td>")
					.append("<td  class=\"tdcontent\">0</td>")
					.append("<td  class=\"tdcontent\">0</td>")
					.append("</tr>")
					.append("<tr><td class=\"label\">Mes Anterior</td>")
					.append("<td  class=\"tdcontent\">0</td>")
					.append("<td  class=\"tdcontent\">0</td>")
					.append("<td  class=\"tdcontent\">0</td>")
					.append("<td  class=\"tdcontent\">0</td>")
	...
	private void htmlUpdate(URL url) {
		...
		else if (url.getPath().equals("/local/ofb")){//faaguilar OFB
			html.setText(createHTML(PAGE_TYPE.PAGE_OFB));
			html.setCaretPosition(0);
			lastUrl = url;
		}
		else if (url != null){
