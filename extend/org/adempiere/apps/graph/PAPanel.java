...
public class PAPanel extends CPanel implements ActionListener
{
	...
	private void init()
	{
		...
		// barChart
		//Box boxH1 = Box.createHorizontalBox();// original faaguilar commented
		Box boxH1 = Box.createVerticalBox();//faaguilar
		...
		// RIGHT, HTML + Bars
	    String ofb_DashBoard = DB.getSQLValueString(null, "Select Value from AD_SysConfig where name='OFB_TABLE_DASHBOARD'");//faaguilar
	    String home="http:///local/home";//faaguilar
	    if(ofb_DashBoard!=null && ofb_DashBoard.equals("Y"))//faaguilar
	    	home="http:///local/ofb";//faaguilar
	    
	    HtmlDashboard contentHtml = new HtmlDashboard(home, m_goals, true);//original code "http:///local/home"  changed by faaguilar 
	    boxV2.add(contentHtml, BorderLayout.CENTER);
	    
	    Box boxH1b = Box.createHorizontalBox();//faaguilar
	    boxH1b.setAlignmentX(LEFT_ALIGNMENT);//faaguilar
	    boxH1b.setAlignmentY(LEFT_ALIGNMENT);//faaguilar
	    
	    //boxH1.setAlignmentX(LEFT_ALIGNMENT);//faaguilar
	    boxH1.add(boxH1b);//faaguilar
	    
	   
		for (int i = 0; i < java.lang.Math.min(6, m_goals.length); i++)//2 original faaguilar change 4
		{
			if(i==3 || i==6)
			{
				boxH1b = Box.createHorizontalBox();//faaguilar
				boxH1b.setAlignmentX(LEFT_ALIGNMENT);//faaguilar
				boxH1b.setAlignmentY(LEFT_ALIGNMENT);//faaguilar
			    boxH1.add(boxH1b);//faaguilar
			}
			...
			{
				Graph g = new Graph(m_goals[i]);
				g.setMaximumSize(new Dimension(330,280));
				boxH1b.add ( g, BorderLayout.CENTER);
				
			}
				//boxH1.add ( new Graph(m_goals[i]), BorderLayout.SOUTH);// original faaguilar commented
		}
	    ...
	    scrollPane2.getViewport().add(boxH1, BorderLayout.NORTH );
	    scrollPane2.setMinimumSize(new Dimension(130, 110));
	    boxV2.add(scrollPane2, BorderLayout.WEST );
	    //faaguilar
