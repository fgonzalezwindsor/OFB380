 ...
 */
public class MutualImportOrderADPostgre extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
			...
			sqlf = new StringBuffer ("UPDATE I_Order o "
					  ...
					  + "AND p.MU_Adherente=o.MU_Adherente  AND p.M_PriceList_ID=o.M_PriceList_ID AND o.AD_Client_ID=p.AD_Client_ID) "
					  + "WHERE o.M_PriceList_Version_ID IS NULL AND I_IsImported<>'Y'").append (clientCheck);
			DB.executeUpdate(sqlf.toString(), get_TrxName());
			//actualizamos rut
			sqlf = new StringBuffer ("UPDATE I_Order o "
					  + "SET BPartnerValue = substring(mu_rut_empresa from 0 for 9) "					  
					  + "WHERE I_IsImported<>'Y'").append (clientCheck);
			DB.executeUpdate(sqlf.toString(), get_TrxName());
