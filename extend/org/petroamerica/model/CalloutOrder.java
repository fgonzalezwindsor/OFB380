...
public class CalloutOrder extends CalloutEngine
{
	...
	public String bPartner (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		String sql = "SELECT p.AD_Language,p.C_PaymentTerm_ID,"
			+ " COALESCE(lship.M_PriceList_ID,p.M_PriceList_ID,g.M_PriceList_ID) AS M_PriceList_ID, p.PaymentRule,p.POReference,"
	...
	public String qty (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
			...
			&& QtyOrdered.signum() > 0)		//	no negative (returns)
		{
			...
			if (product.isStocked() && product.get_ValueAsBoolean("CheckQty"))//faaguilar OFB add && product.get_ValueAsBoolean("CheckQty")
			{
				...
				//faaguilar OFB begin
				BigDecimal available = DB.getSQLValueBD(null, "select  BOMQTYAVAILABLE(M_Product_ID,?,0) from M_Product  " +
						"Where M_Product_ID=? ", M_Warehouse_ID,product.getM_Product_ID());
				
				BigDecimal availableTotal = Env.ZERO;
				String whereClause = "AD_Client_ID=?";
				List<MWarehouse> list = new org.compiere.model.Query(ctx, MWarehouse.Table_Name, whereClause, null)
				...
				for(MWarehouse wh : list)
				{
					...
					availableTotal=availableTotal.add(temp);
				}
				...
				{
					String info = "Disponible Bodega :" + available.toString() +" Disponible Total :"+ availableTotal.toString();//faaguilar OFB
	...
	//faaguilar OFB windsord
	public String qtyLog (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		if (value == null)
			return "";
		...
		if(qty.signum()==0){
			mTab.fireDataStatusEEvent ("El Valor de Cantidad no Debe ser 0", "", false);
			BigDecimal fa=new BigDecimal("99");
			mTab.setValue("FreightAmt",fa);
		}
		else{
	...
	//faaguilar OFB extra discount windsor get from pricelist
	...
	//faaguilar OFB calculate extra discount
	public String extraDiscount (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		if (isCalloutActive() || value == null)
			return "";
		...
		BigDecimal Discount2, Discount3, Discount4, Discount5,PriceActual,PriceList;
		//	get values
		Discount2 = (BigDecimal)mTab.getValue("Discount2");
		Discount3 = (BigDecimal)mTab.getValue("Discount3");
		Discount4 = (BigDecimal)mTab.getValue("Discount4");
		Discount5 = (BigDecimal)mTab.getValue("Discount5");
		log.fine("Discount2=" + Discount2 + ", Discount3=" + Discount3 + ", Discount4=" + Discount4);
		//
		PriceActual = (BigDecimal)mTab.getValue("PriceActual");
		PriceList = (BigDecimal)mTab.getValue("PriceList");
		log.fine("PriceList=" + PriceList);
		

		//	Qty changed - recalc price
		if (mField.getColumnName().equals("Discount2") 
			|| mField.getColumnName().equals("Discount3")
			|| mField.getColumnName().equals("Discount4")
			|| mField.getColumnName().equals("Discount5"))
		{
			PriceActual = new BigDecimal ((100.0 - Discount2.doubleValue()) / 100.0 * PriceList.doubleValue());
			PriceActual = new BigDecimal ((100.0 - Discount3.doubleValue()) / 100.0 * PriceActual.doubleValue());
			PriceActual = new BigDecimal ((100.0 - Discount4.doubleValue()) / 100.0 * PriceActual.doubleValue());
			PriceActual = new BigDecimal ((100.0 - Discount5.doubleValue()) / 100.0 * PriceActual.doubleValue());
			
			
			PriceActual=PriceActual.divide(BigDecimal.ONE  ,BigDecimal.ROUND_HALF_EVEN  );
	...
	public String product (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		mTab.setValue("PriceList", pp.getPriceList());
		mTab.setValue("PriceLimit", pp.getPriceLimit());
		//ininoles se agrega precio costo para PA		
		BigDecimal priceStd = pp.getPriceStd();
		MOrder order = new MOrder(ctx, Env.getContextAsInt(ctx, WindowNo, "C_Order_ID"), null);
		MBPartnerLocation loc = new MBPartnerLocation(ctx, order.getC_BPartner_Location_ID(), null);
		...
		if (Env.isSOTrx(ctx, WindowNo))
		{
			...
			if (product.isStocked() && product.get_ValueAsBoolean("CheckQty"))//faaguilar OFB add && product.get_ValueAsBoolean("CheckQty")
			{
				...
				//faaguilar OFB begin
				BigDecimal available = DB.getSQLValueBD(null, "select  BOMQTYAVAILABLE(M_Product_ID,?,0) from M_Product  " +
						"Where M_Product_ID=? ", M_Warehouse_ID,M_Product_ID.intValue());
	...
	public String bPartnerLocation (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		{
			boolean IsSOTrx = "Y".equals(Env.getContext(ctx, WindowNo, "IsSOTrx"));
			MBPartnerLocation loc = new MBPartnerLocation(ctx, C_BPartnerLocation_ID, null);
			if(IsSOTrx)
				mTab.setValue("M_PriceList_ID", loc.get_ValueAsInt("M_PriceList_ID"));
		}
		return "";
	}	//	bPartner
