 ...
 */
public class ModPAUpCreditDB implements ModelValidator
{
	/**
	 *	Constructor.
	 *	The class is instantiated when logging in and client is selected/known
	 */
	public ModPAUpCreditDB ()
	{
		super ();
	}	//	MyValidator

	/**	Logger			*/
	private static CLogger log = CLogger.getCLogger(ModPAUpCreditDB.class);
	/** Client			*/
	...
	public String docValidate (PO po, int timing)
	{
		...
		{	
			...
			//Se ejecutan update para actualizar credito
			...
			DB.executeUpdate("update c_bpartner set totalopenbalanceofb=coalesce(owncreditlimit,0)+coalesce(externalcreditlimit,0),so_creditusedofb=0" +
					" where C_Bpartner_ID =  ?",order.getC_BPartner_ID(),po.get_TrxName());
			
			if(order.getC_BPartner().isCustomer())
			{				
				...
				DB.executeUpdate("update c_bpartner cbp set so_creditusedofb=Coalesce(( SELECT sum(t.dueamt) AS sum" +
						...
						" where cbp.C_Bpartner_ID =  ?",order.getC_BPartner_ID(),po.get_TrxName());
