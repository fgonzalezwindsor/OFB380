 ...
 */
public class GenerateRefundHeaderOther extends SvrProcess
{	
	/** Warehouse				*/
		...
		try
		{
			...
			while (rsHR.next())
			{
				...
				if(ID_BPartner > 0 && ID_Org > 0)
				{
					...
					else //no existe cabecera de viatico
					{
						//se genera cabecera de viatico
						//se genera viatico
						X_TP_RefundHeader headViatico = new X_TP_RefundHeader(getCtx(), 0, get_TrxName());
						headViatico.setAD_Org_ID(rsHR.getInt("AD_Org_ID"));
