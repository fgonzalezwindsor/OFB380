 ...
 */
public class ModTSMValidQtyReceipt implements ModelValidator
{
	/**
	 *	Constructor.
	 *	The class is instantiated when logging in and client is selected/known
	 */
	public ModTSMValidQtyReceipt ()
	{
		super ();
	}	//	MyValidator
		...
		{	
			...
			if(receipt.isSOTrx() == false)
			{
				if (iLine.getC_OrderLine_ID() > 0)
				{
					...
					String sqlVal= "SELECT SUM(miol.qtyEntered) FROM M_InOutLine miol " +
					" INNER JOIN M_InOut mio on (miol.M_InOut_ID = mio.M_InOut_ID)" +
					" WHERE mio.DocStatus IN ('CO','CL','DR','IP') " +
					" AND mio.Reversal_ID IS NULL" +
					" AND miol.C_OrderLine_ID = "+iLine.getC_OrderLine_ID()+
					" AND miol.M_InOutLine_ID <> "+iLine.get_ID();
					
					BigDecimal qtyTotal = DB.getSQLValueBD(po.get_TrxName(), sqlVal);
					if (qtyTotal == null)
						qtyTotal = Env.ZERO;
					qtyTotal = qtyTotal.add(iLine.getQtyEntered());
					if (qtyTotal.compareTo(oLine.getQtyEntered()) > 0 && receipt.getReversal_ID() <= 0)
					{
