 ...
 */
public class CreateBPMetlife extends SvrProcess
implements ImportProcess
{
	...
	protected String doIt() throws java.lang.Exception
	{
		...
		{	
			if (asignBPartner())
			{
				String sqlC = "SELECT DISTINCT(cc.C_Campaign_ID) as C_Campaign_ID FROM C_Campaign cc " +
						" INNER JOIN c_campaignfollow ccf ON (cc.c_campaign_id = ccf.c_campaign_id) " +
						" WHERE cc.IsOnGoing = 'Y' AND cc.IsActive = 'Y' AND IsCampaignDefault = 'N'" +
						" AND ccf.FinalStatus IN ('IN')";
			
				PreparedStatement pstmtC =  null;
				ResultSet rsC = null;
				try
				{
					pstmtC = DB.prepareStatement(sqlC, get_TrxName());
					rsC = pstmtC.executeQuery();					
					while (rsC.next())
					{
						MCampaign camp = new MCampaign(getCtx(), rsC.getInt("C_Campaign_ID"), get_TrxName());
						if(camp.getName().toLowerCase().contains("mutuaria"))
						{
							asignUserMutuaria(camp.get_ID());
						}
						if(camp.getName().toLowerCase().contains("rentas vitalicias"))
						{
							asignUserRVitalicias(camp.get_ID());
						}
						if(camp.getName().toLowerCase().contains("seguros colectivos"))
						{
							asignUserSColectivos(camp.get_ID());
						}
						if(camp.getName().toLowerCase().contains("dtc"))
						{
							asignUserDMDTC(camp.get_ID());
						}
						if(camp.getName().toLowerCase().contains("agencia"))
						{
							asignUserAgencia(camp.get_ID());
							SubAsignUserAgencia(camp.get_ID());
						}
						if(camp.getName().toLowerCase().contains("consumo"))
						{
							asignUserCConsumo(camp.get_ID());
						}						
					}
				}
	...
	{		
		...
		//reemplazamos los apostrofes para que no de problemas con los valores de etiqueta de region
		sql = new StringBuffer ("UPDATE i_bpartnerxml set IA_REGION = replace(ia_region,'''','') "
				+ " WHERE I_IsImported<>'Y' AND I_IsAsigned<>'Y'").append(clientCheck);
		no = DB.executeUpdateEx(sql.toString(), get_TrxName());
		log.config("replace apostrofe: " + no);
	...
	public Boolean asignBPartner() throws SQLException
	{
		...
		try
		{
			...
			{	
				...
				int existeCampaña = 0;
				//nuevo campo para no repetir los lead
				boolean flag = false;
				while (rsEtiqueta.next())
				{
					...
					String sqlCadenasUrl = "SELECT iav.value, cc.C_Campaign_ID FROM C_Campaign cc INNER JOIN R_CampaignInterest ci ON (cc.C_Campaign_ID = ci.C_Campaign_ID) "+
							...
							"WHERE IsOnGoing = 'Y' AND ci.IsCapaignFilter = 'Y' AND iav.R_InterestArea_ID=?";
					...
					{	
						...
						if (valueIABP!= null && valueUrl != null)
						{
							...
							if (valueIABP.contains(valueUrl))
							{
								...
								// Se modifica codigo para que siempre agrege los lead aunque se repitan en la campaña
								...
								if(!flag)
								{
									...
									cFollow.save();
									//marcamos que ya se creo el leas
									flag = true;
	...
	{	
		...
		//@mfrojas Cambio en la consulta. Se agrega el "FinalStatus='AS'" al count, pues originalmente estaba contando
		//todos los leads, y Aldo menciona que este conteo es sólo de los AS
		
		String sqlEje = "SELECT DISTINCT (cf.AD_User_ID), COALESCE((SELECT COUNT(1) FROM C_CampaignFollow " +
		" WHERE C_Campaign_ID = cf.C_Campaign_ID AND AD_User_ID = cf.AD_User_ID and FinalStatus = cf.FinalStatus),0) " +
	    " as count FROM C_CampaignFollow cf" +
		" WHERE cf.C_Campaign_ID = ? AND cf.AD_User_ID IS NOT NULL AND FinalStatus = 'AS'";
		...
		DB.close(rsEt, pstmtEt);
		rsEt = null; pstmtEt = null;
		...
		try
		{
			...
			while (rsCLines.next())
			{
				...
				DB.close(rsEtBP, pstmtEtBP);
				rsEtBP = null; pstmtEtBP = null;
				...
				}			//@mfrojas se agrega el and finalstatus='AS' a la consulta (dentro del coalesce)		
				String sqlEtUser = "SELECT DISTINCT(ui.AD_User_ID), " +
						" COALESCE((SELECT COUNT(1) FROM C_CampaignFollow WHERE C_Campaign_ID = "+cFollow.getC_Campaign_ID()+" AND FinalStatus = 'AS' AND AD_User_ID = ui.AD_User_ID),0) as cant " +
						" FROM R_UserInterest ui " +						
						" INNER JOIN AD_User adu ON (ui.AD_User_ID = adu.AD_User_ID) "+
						" WHERE adu.IsActive = 'Y' AND adu.ActivePool = 'N' AND ui.IsActive = 'Y' " +
						" AND R_InterestArea_ID IN ("+id_IaBPsql+") AND (R_InterestAreaValues_ID IN ("+id_IaVBPsql+") OR R_InterestAreaValues_ID IS NULL) ";
				if (InterestAreaBP2_ID > 0)
				{
				...
				DB.close(rsEtUser, pstmtEtUser);
				rsEtUser = null; pstmtEtUser = null;
				...
				if (ID_EjecutivoM > 0)
				{
					...
					try
					{
						...
						log.config("No se pudo setear variable AsignMail");
					}
					cFollow.save();
					...
					//se hace commit para que no de error
					commitEx();
					//ininoles mandamos correo					
					//sendMailCFollow(cFollow.get_ID());
					//ininoles end
				}												
			}	
		}
		...
		*/
		...
		try
		{
			...
			while (rsCLinesP1.next())
			{
				...
				String sqlEtUser = "SELECT DISTINCT(ui.AD_User_ID) as AD_User_ID, adu.qtyLead " +								
						" FROM R_UserInterest ui " +
						" INNER JOIN AD_User adu ON (ui.AD_User_ID = adu.AD_User_ID) "+
				...
				sqlEtUser = sqlEtUser + getValidUserCampaign(cam.get_ID());						
				...
				int cantIndUser1 = DB.getSQLValue(get_TrxName(), "SELECT COUNT(1) FROM AD_User " +
						"WHERE EXISTS ("+sqlEtUser.replace("ORDER BY adu.qtyLead desc", " ")+")");					
				if (cantIndUser1 < 1)
				{
					String sql1 = "UPDATE AD_User SET UseAgencia = 'N' WHERE EXISTS ("+sqlEtUser.replace("AND adu.UseAgencia = 'N' AND adu.qtyLead > 0", " ")+")";
					DB.executeUpdate(sql1.replace("ORDER BY adu.qtyLead desc", " "), get_TrxName());
					commitEx();
				}
				...
				if (cantIndUser2 < 1)
				{
					...
					DB.executeUpdate(sql2.replace("ORDER BY adu.qtyLead desc", " "), get_TrxName());
					commitEx();
				}						
		...
		try
		{
			...
			while (rsCLinesP2.next())
			{
				...
				//sobreescribimos InterestArea3_ID con valor de ciudad para que tambien actue como filtro.
				InterestArea3_ID = DB.getSQLValue(get_TrxName(), "SELECT MAX(R_InterestArea_ID) FROM R_InterestArea WHERE UPPER(value) like 'CIUDAD'");
				String sqlEtBP = "SELECT R_InterestArea_ID, MAX(R_InterestAreaValues_ID) as R_InterestAreaValues_ID FROM R_ContactInterest "+
				"WHERE C_Bpartner_ID = ? AND R_InterestArea_ID IN ("+InterestArea1_ID+","+InterestArea2_ID+","+InterestArea3_ID+")" +
						" GROUP BY R_InterestArea_ID ";
	...
	{	
		...
		*/
		...
		log.config("Etiqueta: "+ID_EtiquetaUrlGL); //mfrojas 
		//sobreescribimos id de variable etiqueta URL ininoles en vacaciones :D
		int ID_EtiquetaURL = DB.getSQLValue(get_TrxName(), "SELECT MAX(R_InterestArea_ID) FROM R_InterestArea WHERE value like 'LeadSitioPrivado'");
		if (ID_EtiquetaURL > 0)
			ID_EtiquetaUrlGL = ID_EtiquetaURL;
		...
		try
		{
			...
			while (rsCLines.next())
			{
				...
				String sqlEtUser = "SELECT DISTINCT(ui.AD_User_ID), " +
						...
						" FROM R_UserInterest ui " +
						" INNER JOIN ad_user adu ON (ui.ad_user_ID = adu.ad_user_ID) "+
						" WHERE ui.IsActive = 'Y' AND adu.IsActive = 'Y' " +
						" AND R_InterestArea_ID IN ("+id_IaBPsql+") AND (R_InterestAreaValues_ID IN ("+id_IaVBPsql+") OR R_InterestAreaValues_ID IS NULL) ";
				if (InterestAreaBP2_ID > 0)
				{
					sqlEtUser = sqlEtUser + " AND R_InterestAreaRef2_ID IN ("+id_IaBPsql+") AND (R_InterestAreaValues2_ID IN ("+id_IaVBPsql+") OR R_InterestAreaValues2_ID IS NULL)";
	...
	{
		String sqlUC = " AND ui.AD_User_ID IN " +
				"(SELECT AD_USER_ID FROM ad_usercampaign WHERE IsActive = 'Y' AND C_Campaign_ID = "+ID_CampaignV+") ";		
