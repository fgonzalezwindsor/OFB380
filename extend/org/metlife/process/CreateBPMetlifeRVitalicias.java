 ...
 */
public class CreateBPMetlifeRVitalicias extends SvrProcess
implements ImportProcess
{
	...
	public Boolean asignBPartner() throws SQLException
	{
		...
		sql = new StringBuffer ("SELECT C_Bpartner_ID, I_BPartnerXML_ID FROM I_BPartnerXML " +
				"WHERE I_IsImported='Y' AND I_IsAsigned='N' AND C_Bpartner_ID IS NOT NULL AND IsActive = 'Y' ").append(clientCheck);
				...
				*/
				...
				try
				{
					...
					while (rsCLinesP1.next())
					{
						...
						int cantIndUser1 = DB.getSQLValue(get_TrxName(), "SELECT COUNT(1) FROM AD_User " +
								"WHERE EXISTS ("+sqlEtUser+")");					
						if (cantIndUser1 < 1)
						{
							DB.executeUpdate("UPDATE AD_User SET UseAgencia = 'N' WHERE EXISTS ("+sqlEtUser.replace("AND adu.UseAgencia = 'N' AND adu.qtyLead > 0", " ")+")", get_TrxName());
							commitEx();
						}
						...
						if (cantIndUser2 < 1)
						{
							DB.executeUpdate("UPDATE AD_User SET qtyLead = qtyLeadBase WHERE EXISTS ("+sqlEtUser.replace("AND adu.UseAgencia = 'N' AND adu.qtyLead > 0", " ")+")", get_TrxName());
							commitEx();
						}						
				...
				try
				{
					...
					while (rsCLinesP2.next())
					{
						...
						if (cantIndUser1 < 1)
						{
							DB.executeUpdate("UPDATE AD_User SET UseAgencia = 'N' WHERE AD_User_ID IN ("+sqlEtUser.replace("AND adu.UseAgencia = 'N' AND adu.qtyLead > 0", " ")+")", get_TrxName());
							commitEx();
						}
						int cantIndUser2 = DB.getSQLValue(get_TrxName(), "SELECT COUNT(1) FROM AD_User " +
								"WHERE EXISTS ("+sqlEtUser+")");					
						if (cantIndUser2 < 1)
						{
							DB.executeUpdate("UPDATE AD_User SET qtyLead = qtyLeadBase WHERE AD_User_ID IN ("+sqlEtUser.replace("AND adu.UseAgencia = 'N' AND adu.qtyLead > 0", " ")+")", get_TrxName());
							commitEx();
						}						
						//fin validaciones y actualizaciones
	...
	public String getValidUserCampaign(int ID_CampaignV)
	{
		String sqlUC = " AND ui.AD_User_ID IN " +
				"(SELECT AD_USER_ID FROM ad_usercampaign WHERE C_Campaign_ID = "+ID_CampaignV+") ";		
