...
public class ModPAUpdateCreditBP implements ModelValidator
{
	...
	public String docValidate (PO po, int timing)
	{
		...
		{	
			...
			for (int i = 0; i < lines.length; i++)
			{
				...
				{	
					...
					BigDecimal amt = line.getC_OrderLine().getPriceEntered().multiply(line.getQtyEntered());
					BigDecimal taxAmt = Env.ZERO;
					/*if(oLine.getC_Tax().getRate().compareTo(Env.ZERO) > 0)
						taxAmt = amt.multiply(oLine.getC_Tax().getRate()).divide(Env.ONEHUNDRED).setScale(oLine.getC_Order().getC_Currency().getStdPrecision(),BigDecimal.ROUND_HALF_EVEN);
					if(taxAmt == null)
						taxAmt = Env.ZERO;*/
					/*amt = amt.add(taxAmt);
					BigDecimal fixedTax = (BigDecimal)oLine.get_Value("FixedTax");
					BigDecimal variableTax = (BigDecimal)oLine.get_Value("VariableTax");
					BigDecimal ivaTax = (BigDecimal)oLine.get_Value("IVATaxAmt");					
					if(fixedTax != null)
						amt = amt.add(fixedTax.multiply(line.getQtyEntered()));
					if(variableTax != null)
						amt = amt.add(variableTax.multiply(line.getQtyEntered()));										
					if(ivaTax != null)
						amt = amt.add(ivaTax.divide(oLine.getQtyOrdered()).multiply(line.getMovementQty()));
					amtAcum = amtAcum.add(amt);
				}
			}
			MBPartner bp = new MBPartner(po.getCtx(), InOut.getC_BPartner_ID(), po.get_TrxName());
			BigDecimal amtTotal = (BigDecimal)bp.get_Value("TotalOpenBalanceOFB");
			amtTotal = amtTotal.subtract(amtAcum);
			bp.set_CustomColumn("TotalOpenBalanceOFB", amtTotal);
			BigDecimal amtUsed = (BigDecimal)bp.get_Value("SO_CreditUsedOFB");
			if(amtUsed == null)
				amtUsed = Env.ZERO;
			amtUsed = amtUsed.add(amtAcum);
		...
		//se aumenta credito con la nota de venta
		if(timing == TIMING_AFTER_COMPLETE && po.get_Table_ID()==MInvoice.Table_ID) 
		{	
			MInvoice inv = (MInvoice)po;
