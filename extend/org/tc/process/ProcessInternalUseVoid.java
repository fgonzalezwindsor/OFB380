...
{	
	...
	{	
		String sqlDet = "SELECT m_inventory_id from m_inventory where docstatus='CO' AND "+
				" created > '2017-06-28' and created < '2017-06-29' and m_inventory_id in (select "+
				"m_inventory_id from m_inventoryline where qtyinternaluse = 0)";
		...
		try
		{
			...
			while (rs.next ())
			{
					...
					inv.voidIt();
					inv.setDescription(inv.getDescription()+" -- Anulado");
					inv.save();

					cant++;
					log.config("cantidad actual: "+cant);
			}					
			log.config("Se ha Anulado "+cant+" consumos ");
