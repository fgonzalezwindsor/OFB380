...
public class ImportInvoice extends SvrProcess
{
	...
	protected String doIt() throws java.lang.Exception
	{
		...
		**/
		...
		//faaguilar OFB new columns begin
//		IDUser
		sql = new StringBuffer ("UPDATE I_Invoice o "
			  + "SET C_BPartnerC_ID=(SELECT MAX(C_BPartner_ID) FROM C_BPartner bp"
			  + " WHERE o.BPartnerValueC=bp.Value AND o.AD_Client_ID=bp.AD_Client_ID) "
			  + "WHERE C_BPartnerC_ID IS NULL AND BPartnerValueC IS NOT NULL"
			  + " AND I_IsImported<>'Y'").append (clientCheck);
		no = DB.executeUpdate(sql.toString(), get_TrxName());
		
//		A_Asset_ID2 
		sql = new StringBuffer ("UPDATE I_Invoice o "
				  + "SET A_Asset2_ID=(SELECT MAX(A_Asset_ID) FROM A_Asset bp"
				  + " WHERE o.AssetValue=bp.Value AND o.AD_Client_ID=bp.AD_Client_ID) "
				  + "WHERE A_Asset2_ID IS NULL AND AssetValue IS NOT NULL"
				  + " AND I_IsImported<>'Y'").append (clientCheck);
			no = DB.executeUpdate(sql.toString(), get_TrxName());
		//faaguilar OFB new columns end
		...
		try
		{
			...
			while (rs.next ())
			{
				...
				//faaguilar OFB new columns begin geminis
				line.set_ValueOfColumn("C_BPartnerC_ID", imp.get_Value("C_BPartnerC_ID"));
				line.set_ValueOfColumn("A_Asset2_ID", imp.get_Value("A_Asset2_ID"));
				line.set_ValueOfColumn("C_ProjectOFB_ID", imp.get_Value("C_ProjectOFB_ID"));
				line.set_ValueOfColumn("Other_Info_1", imp.get_Value("Other_Info_1"));
				line.set_ValueOfColumn("Other_Info_2", imp.get_Value("Other_Info_2"));
				line.set_ValueOfColumn("Other_Info_3", imp.get_Value("Other_Info_3"));				
				line.setAD_Org_ID(imp.getAD_Org_ID());
				//faaguilar OFB new columns end geminis
				//ininoles nuevo campo para cpv
