 ...
 */
public class CalloutCPVInvoice extends CalloutEngine
{
	/**
	 *
	 *  @param ctx      Context
	...
	public String budget (Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value)
	{
		...
		if(mTab.getValue("M_Product_ID") != null && (Integer)mTab.getValue("M_Product_ID")>0)
			idProduct = (Integer)mTab.getValue("M_Product_ID");		
		BigDecimal amtline = (BigDecimal)mTab.getValue("PriceEntered");
			...
			if (!(amtline.compareTo(Env.ZERO) > 0))
			{
				...
				try 
				{
					...
					if (rs.next())						
					{
						...
						mTab.setValue("QtyEntered", rs.getBigDecimal("Qty"));
						mTab.setValue("QtyInvoiced", rs.getBigDecimal("Qty"));
						mTab.setValue("PriceEntered", rs.getBigDecimal("PriceActual"));
