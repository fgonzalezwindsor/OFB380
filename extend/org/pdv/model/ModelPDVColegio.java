...
public class ModelPDVColegio implements ModelValidator
{
	...
	public String modelChange (PO po, int type) throws Exception
	{
		...
		if(  type == TYPE_AFTER_CHANGE && po.get_Table_ID()==MOrder.Table_ID && po.is_ValueChanged("C_PaymentTerm_ID")) // Cambio T�rmino de Pago Matr�cula
		{
			...
			/*for(MOrderLine line : order.getLines())
			{
				if(line.getM_Product_ID()>0)
					if(line.getM_Product().getM_Product_Category().getName().toUpperCase().indexOf("MATR�CULA")>=0 || line.getM_Product().getM_Product_Category().getName().toUpperCase().indexOf("MATRICULA")>=0)
					{
		...
		if(  type == TYPE_AFTER_CHANGE && po.get_Table_ID()==MOrder.Table_ID && po.is_ValueChanged("C_PaymentTerm2_ID")) // T�rmino de Pago Colegiatura
		{
		...
		if(type == TYPE_BEFORE_NEW && po.get_Table_ID()==MInvoicePaySchedule.Table_ID)
		{
			...
			{
				MInvoice in =new MInvoice(is.getCtx(), is.getC_Invoice_ID(), is.get_TrxName());
		...
		//cuotas para compromisos adquiridos		
		if(type == TYPE_BEFORE_NEW && po.get_Table_ID()==MInvoicePaySchedule.Table_ID)
		{			
			MInvoicePaySchedule is = (MInvoicePaySchedule)po;
			MInvoice inv = new MInvoice(po.getCtx(), is.getC_Invoice_ID(), po.get_TrxName());
			if (inv.getC_DocTypeTarget().getDocBaseType().compareTo("APA")==0)
			{				
				if(is.get_Value("ForeignPrice")==null)
				{
					//MInvoice in =new MInvoice(is.getCtx(), is.getC_Invoice_ID(), is.get_TrxName());
					if(inv.get_Value("ForeignPrice")!=null && ((BigDecimal)inv.get_Value("ForeignPrice")).signum()>0)
					{
						is.set_CustomColumn("ForeignPrice", ((BigDecimal)inv.get_Value("ForeignPrice")).multiply( is.getC_PaySchedule().getPercentage().divide(Env.ONEHUNDRED, BigDecimal.ROUND_HALF_UP)    ));
					}
					else if (is.getDueAmt().compareTo(Env.ZERO) > 0 && inv.getC_Currency_ID() != 228)
					{
							is.set_CustomColumn("ForeignPrice", (is.getDueAmt()));
					}
					else
					{
						BigDecimal amt = DB.getSQLValueBD(po.get_TrxName(), "SELECT SUM(ForeignPrice) FROM C_InvoiceLine WHERE C_Invoice_ID = ?", inv.get_ID());
						if(amt.compareTo(Env.ZERO) > 0)
							is.set_CustomColumn("ForeignPrice", amt.multiply( is.getC_PaySchedule().getPercentage().divide(Env.ONEHUNDRED, BigDecimal.ROUND_HALF_UP)    ));
					}
				}
			}
		}
	...
	public String docValidate (PO po, int timing)
	{
		...
		if(timing == TIMING_AFTER_COMPLETE && po.get_Table_ID()==MInvoice.Table_ID)
		{
			...
			//ininoles se agrega condicion para importar pagares y que no se borren las cuotas al completar
			if(inv.isSOTrx() && inv.getC_DocType().getDocBaseType().equals("CDC") && inv.getC_Order_ID() > 0)//pagare 
			{
				DB.executeUpdate("Delete from C_InvoicePaySchedule where C_Invoice_ID="+inv.getC_Invoice_ID(), po.get_TrxName());
				PreparedStatement pstmt = null;
				try
