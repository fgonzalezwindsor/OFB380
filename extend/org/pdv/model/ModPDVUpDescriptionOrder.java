 ...
 */
public class ModPDVUpDescriptionOrder implements ModelValidator
{
	/**
	 *	Constructor.
	 *	The class is instantiated when logging in and client is selected/known
	 */
	public ModPDVUpDescriptionOrder ()
	{
		super ();
	}	//	MyValidator

	/**	Logger			*/
	private static CLogger log = CLogger.getCLogger(ModPDVUpDescriptionOrder.class);
	/** Client			*/
	...
	public String modelChange (PO po, int type) throws Exception
	{
		...
		{	
			...
			{
				int id_User = 0;
				if(oLine.get_ValueAsInt("AD_User_ID") > 0)
					id_User = oLine.get_ValueAsInt("AD_User_ID");
				else
					id_User = 0;
				if(id_User > 0)
				{
					MUser user = new MUser(po.getCtx(), id_User, po.get_TrxName());
					DB.executeUpdate("UPDATE C_OrderLine SET Description = Description || ' Alumno:"+user.getName()
