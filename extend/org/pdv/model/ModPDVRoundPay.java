 ...
 */
public class ModPDVRoundPay implements ModelValidator
{
	/**
	 *	Constructor.
	 *	The class is instantiated when logging in and client is selected/known
	 */
	public ModPDVRoundPay ()
	{
		super ();
	}	//	MyValidator
	...
	public String modelChange (PO po, int type) throws Exception
	{
		...
		if( (type == TYPE_AFTER_CHANGE || type == TYPE_AFTER_NEW) && po.get_Table_ID()==MPayment.Table_ID) // Cambio Tï¿½rmino de Pago Matrï¿½cula
		{
			MPayment pay = (MPayment)po;
			if (pay.isReceipt() && pay.getTenderType().compareToIgnoreCase("K") == 0)
			{
				BigDecimal d = pay.getPayAmt().divide(Env.ONEHUNDRED);
				BigDecimal result = d.subtract(d.setScale(0, RoundingMode.FLOOR)).movePointRight(d.scale());      
				if (result.compareTo(Env.ZERO) != 0)
				{
					if(result.compareTo(BigDecimal.valueOf(50)) >= 0 )
					{
