 ...
 */
public class ModDPPUpdateAssetInvoice implements ModelValidator
{
	/**
	 *	Constructor.
	 *	The class is instantiated when logging in and client is selected/known
	 */
	public ModDPPUpdateAssetInvoice ()
	{
		super ();
	}	//	MyValidator
	...
	public String docValidate (PO po, int timing)
	{
		...
		if(timing == TIMING_AFTER_COMPLETE && po.get_Table_ID()==MInvoice.Table_ID)
		{
			...
			{
				MDocType docType = new MDocType(po.getCtx(), inv.getC_DocTypeTarget_ID(), po.get_TrxName());
				if (docType.get_ValueAsString("Lot") != null)
				{
					//@mfrojas se agrega validación para los tipos de documento que no deben hacer cambios (deben tener "SIN" en la referencia (sin acción))
					if (docType.get_ValueAsString("Lot").compareTo("SIN") != 0)
					{
